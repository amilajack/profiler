...
import { TooltipDetail, TooltipDetailComponent } from './TooltipDetails';
...
export function getGCMinorDetails(data: GCMinorMarkerPayload): TooltipDetailComponent[] {
  const details: Array<TooltipDetailComponent> = [];
  if (data.nursery) {
...
export function getGCMajorDetails(data: GCMajorMarkerPayload): TooltipDetailComponent[] {
  const details: Array<TooltipDetailComponent> = [];
...
export function getGCSliceDetails(data: GCSliceMarkerPayload): TooltipDetailComponent[] {
  const details: Array<TooltipDetailComponent> = [];
  const timings = data.timings;
  let triggers = null;
...
function _makePhaseTimesArray(phases: PhaseTimes<Microseconds>): Array<PhaseTimeTuple> {
  const array: Array<PhaseTimeTuple> = [];
...
function _treeInsert(
  ...
  const uninitializedPhaseTime = { name: '(temp)', time: 0 } as const;

  let node = tree.get(component);
  if (!node) {
    // Make a new node and grow the tree in this direction.
    const value = { ...uninitializedPhaseTime } as const;
...
function _treeGetLeaves(tree: Map<string, PhaseTreeNode>): Array<PhaseTimeTuple> {
  const leaves: Array<PhaseTimeTuple> = [];
...
function _forEachTreeNode(tree: Map<string, PhaseTreeNode>, visit: (node: PhaseTreeNode) => void): void {
  ...
  _forEachTreeNode(
    ...
    ({ leaf, branches, value: { time: totalTime, name } }) => {
      if (!leaf) {
        ...
        const value = { name: name + selfTimeSuffix, time: remaining } as const;
