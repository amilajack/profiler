...
type RenderedComponentSettings = {
  ...
  readonly treeOrderSampleComparator: (arg1: IndexIntoSamplesTable, arg2: IndexIntoSamplesTable) => number | null | undefined,
...
export class ActivityFillGraphQuerier {
   ...
   */
  getSampleAndCpuRatioAtClick(cssX: CssPixels, cssY: CssPixels, time: Milliseconds): HoveredPixelState | null {
    const {
      rangeFilteredThread: { samples, stackTable },
      greyCategoryIndex,
    } = this.renderedComponentSettings;
   ...
   */
  _getSamplesAtTime(time: Milliseconds): ReadonlyArray<SampleContributionToPixel> {
    ...
    const sampleContributions: Array<any | {
      readonly contribution: number,
      readonly sample: IndexIntoSamplesTable
    }> = [];
   ...
   * been applied.
   */
  _getSmoothedContributionFromSampleToPixel(xPixel: number, sample: IndexIntoSamplesTable): number {
    const {
      rangeFilteredThread: { samples },
      enableCPUUsage,
      interval,
