 ...
 */
import {EditorView, Decoration, lineNumbers, GutterMarker, gutter, gutterLineClass} from '@codemirror/view';
import {
  EditorState,
  StateField,
  StateEffect,
...
function _languageExtForPath(path: string | null): any /* LanguageSupport | [] */ {
  if (path === null) {
    return [];
  }
  if (path.endsWith('.rs')) {
...
const timingsField = StateField.define<LineTimings>({
  ...
  update(timings: any, transaction: any) {
