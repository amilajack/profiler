...
import type {StartEndRange, Marker, Milliseconds} from 'firefox-profiler/types';
...
A extends any, // eslint-disable-next-line flowtype/no-weak-types
B extends any>(a: A, b: B): boolean {
  let aLength = 0;
  let bLength = 0;
  for (const key in a) {
 ...
 */
export function getStartEndRangeForMarker(rangeStart: Milliseconds, rangeEnd: Milliseconds, marker: Marker): StartEndRange {
 ...
 */
export function getTrackSelectionModifier(event: MouseEvent | KeyboardEvent | React.MouseEvent | React.KeyboardEvent): 'ctrl' | 'none' {
