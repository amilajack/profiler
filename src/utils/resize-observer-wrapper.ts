...
function createResizeObserverWrapper() {
  ...
  const callbacks: Map<Element, Set<() => unknown>> = new Map();
  ...
  return {
    subscribe(element: HTMLElement, callback: () => unknown) {
    ...
    unsubscribe(element: HTMLElement, callback: () => unknown) {
...
type ResizeObserverWrapper = {
  subscribe: (elt: HTMLElement, callback: () => unknown) => void,
  unsubscribe: (elt: HTMLElement, callback: () => unknown) => void
};
