 ...
 */
export function finalizeFullProfileView(profile: Profile, maybeSelectedThreadIndexes: Set<ThreadIndex> | null): ThunkAction<void> {
...
export function finalizeOriginProfileView(
  ...
  return (dispatch: Dispatch) => {
    ...
    const originsTimelineNoOrigin: Array<any | {
 ...
 */
export function changeTimelineTrackOrganization(timelineTrackOrganization: TimelineTrackOrganization): ThunkAction<void> {
  return (dispatch: Dispatch, getState: GetState) => {
...
export function bulkProcessSymbolicationSteps(symbolicationStepsPerThread: Map<ThreadIndex, SymbolicationStepInfo[]>): ThunkAction<void> {
  return (dispatch: Dispatch, getState: GetState) => {
...
if (typeof window === 'object' && window.requestIdleCallback) {
  ...
  requestIdleCallbackPolyfill = (callback: () => void) => setTimeout(callback, 0);
}
 ...
 */
async function _unpackGeckoProfileFromBrowser(profile: ArrayBuffer | MixedObject): Promise<MixedObject> {
...
export async function doSymbolicateProfile(
  ...
  const completionPromises: Array<Promise<undefined> | any> = [];
...
function _wait(delayMs: number): Promise<void> {
  return new Promise((resolve: (result: Promise<undefined> | undefined) => void) => setTimeout(resolve, delayMs));
}

function _loadProbablyFailedDueToSafariLocalhostHTTPRestriction(url: string, error: Error): boolean {
...
type FetchProfileArgs = {
  url: string,
  onTemporaryError: (arg1: TemporaryError) => void,
 ...
 */
async function _extractZipFromResponse(response: Response, reportError: (...data: Array<any>) => void): Promise<JSZip> {
 ...
 */
async function _extractJsonFromArrayBuffer(arrayBuffer: ArrayBuffer): Promise<MixedObject> {
...
export function retrieveProfileForRawUrl(location: Location, browserConnectionStatus?: BrowserConnectionStatus): ThunkAction<Promise<Profile | null>> {
  return async (dispatch: Dispatch, getState: GetState) => {
