 ...
 */
export async function withHistoryReplaceStateAsync(fn: () => Promise<void>): Promise<void> {
  _enableHistoryReplaceState();
  try {
    await fn();
  } finally {
...
type ActiveTabProfileSpecificBaseQuery = {
  ...
  ctxId: TabID | undefined
};

// Base query that only applies to origins profile view.
type OriginsProfileSpecificBaseQuery = Record<any, any>;
...
type BaseQuery = {
  ...
  timelineType: string,
  sourceView: string
} & (FullProfileSpecificBaseQuery) & (ActiveTabProfileSpecificBaseQuery) & (OriginsProfileSpecificBaseQuery);
...
type $MakeOptional = <T>(arg1: T) => T | undefined;
// Base query shape is needed for the typechecking during the URL query initialization.
type BaseQueryShape = Partial<Flow.ObjMap<BaseQuery, $MakeOptional>>;
...
type FullProfileSpecificBaseQueryShape = Partial<Flow.ObjMap<FullProfileSpecificBaseQuery, $MakeOptional>>;
type ActiveTabProfileSpecificBaseQueryShape = Partial<Flow.ObjMap<ActiveTabProfileSpecificBaseQuery, $MakeOptional>>;
type OriginsProfileSpecificBaseQueryShape = Partial<Flow.ObjMap<OriginsProfileSpecificBaseQuery, $MakeOptional>>;

// Query shapes for individual query paths. These are needed for QueryShape union type.
type CallTreeQueryShape = Partial<Flow.ObjMap<CallTreeQuery, $MakeOptional>>;
type MarkersQueryShape = Partial<Flow.ObjMap<MarkersQuery, $MakeOptional>>;
type NetworkQueryShape = Partial<Flow.ObjMap<NetworkQuery, $MakeOptional>>;
type StackChartQueryShape = Partial<Flow.ObjMap<StackChartQuery, $MakeOptional>>;
type JsTracerQueryShape = Partial<Flow.ObjMap<JsTracerQuery, $MakeOptional>>;
...
export function getQueryStringFromUrlState(urlState: UrlState): string {
  ...
  switch (timelineTrackOrganization.type) {
    ...
    case 'origins':
      baseQuery = ({} as OriginsProfileSpecificBaseQueryShape);
      break;
    default:
      throw assertExhaustiveCheck(
        timelineTrackOrganization,
...
export function ensureIsValidDataSource(possibleDataSource?: string): DataSource {
  // By casting `possibleDataSource` to a DataSource beforehand, we let Flow
  // enforce that we look at all possible values.
  const coercedDataSource = coerce<string, DataSource>(possibleDataSource || 'none');
 ...
 */
export function stateFromLocation(location: Location, profile?: Profile | null): UrlState {
  ...
  const isBottomBoxOpenPerPanel: Record<string, any> = {};
...
function convertGlobalTrackOrderFromString(rawString?: string | null): TrackIndex[] {
  if (!rawString) {
    return [];
  }
...
function convertGlobalTrackOrderToString(order: TrackIndex[]): string | undefined {
  return encodeUintArrayForUrlComponent(order) || undefined;
}

function convertHiddenGlobalTracksFromString(rawString?: string | null): Set<TrackIndex> {
  if (!rawString) {
    return new Set();
  }
...
function convertHiddenGlobalTracksToString(hiddenGlobalTracks: Set<TrackIndex>): string | undefined {
 ...
 */
function convertHiddenLocalTracksByPidFromString(rawText?: string | null): Map<Pid, Set<TrackIndex>> {
  if (!rawText) {
    return new Map();
  }
...
function convertHiddenLocalTracksByPidToString(hiddenLocalTracksByPid: Map<Pid, Set<TrackIndex>>): string | undefined {
  const strings: Array<any | string> = [];
 ...
 */
function convertLocalTrackOrderByPidFromString(rawText?: string | null): Map<Pid, TrackIndex[]> {
  if (!rawText) {
    return new Map();
  }
...
function convertLocalTrackOrderByPidToString(localTrackOrderByPid: Map<Pid, TrackIndex[]>): string | undefined {
  const strings: Array<any | string> = [];
...
const _upgraders: {
  [key: number]: (location: ProcessedLocationBeforeUpgrade, profile?: Profile) => void
} = {
  ...
  [5]: ({
    query,
  }: ProcessedLocationBeforeUpgrade) => {
...
function validateTimelineTrackOrganization(type: string | null | undefined, tabID: number | null): TimelineTrackOrganization {
 ...
 */
function validateTimelineType(type?: string | null): TimelineType {
  // Pretend this is a TimelineTrackOrganization so that we can exhaustively
  // go through each option.
