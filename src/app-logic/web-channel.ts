...
export type Request = StatusQueryRequest | EnableMenuButtonRequest | GetProfileRequest | GetSymbolTableRequest | QuerySymbolicationApiRequest;

type StatusQueryRequest = {
  type: 'STATUS_QUERY'
};
type EnableMenuButtonRequest = {
...
export type MessageFromBrowser<R extends ResponseFromBrowser> = OutOfBandErrorMessageFromBrowser | ErrorResponseMessageFromBrowser | SuccessResponseMessageFromBrowser<R>;
...
export type ResponseFromBrowser = StatusQueryResponse | EnableMenuButtonResponse | GetProfileResponse | GetSymbolTableResponse | QuerySymbolicationApiResponse;
...
type EnableMenuButtonResponse = undefined;
...
declare function _sendMessageWithResponse: (arg1: StatusQueryRequest) => Promise<StatusQueryResponse>;
declare function _sendMessageWithResponse: (arg1: EnableMenuButtonRequest) => Promise<EnableMenuButtonResponse>;
declare function _sendMessageWithResponse: (arg1: GetProfileRequest) => Promise<GetProfileResponse>;
declare function _sendMessageWithResponse: (arg1: GetSymbolTableRequest) => Promise<GetSymbolTableResponse>;
declare function _sendMessageWithResponse: (arg1: QuerySymbolicationApiRequest) => Promise<QuerySymbolicationApiResponse>;
...
export async function getSymbolTableViaWebChannel(debugName: string, breakpadId: string): Promise<SymbolTableAsTuple> {
...
export async function getProfileViaWebChannel(): Promise<ArrayBuffer | MixedObject> {
  return _sendMessageWithResponse({
    type: 'GET_PROFILE',
  });
}

export async function querySymbolicationApiViaWebChannel(path: string, requestJson: string): Promise<string> {
  return _sendMessageWithResponse({
    type: 'QUERY_SYMBOLICATION_API',
    path,
    requestJson,
...
function _sendMessageWithResponse(Request: Request): Promise<ResponseFromBrowser> {
  const requestId = _requestId++;
  const type = Request.type;

  return new Promise((resolve, reject: (error?: any) => void) => {
    function listener(event: WebChannelEvent) {
