...
export type BrowserConnectionStatus = // The initial state.
...
class BrowserConnectionImpl implements BrowserConnection {
  ...
  async querySymbolicationApi(path: string, requestJson: string): Promise<string> {
  ...
  async getSymbolTable(debugName: string, breakpadId: string): Promise<SymbolTableAsTuple> {
...
function makeTimeoutRejectionPromise(durationInMs: number) {
  return new Promise((_resolve: (result: Promise<never>) => void, reject: (error?: any) => void) => {
    setTimeout(() => {
...
export async function createBrowserConnection(userAgent: string = navigator.userAgent): Promise<BrowserConnectionStatus> {
