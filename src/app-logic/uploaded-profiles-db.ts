...
export type UploadedProfileInformation = {
  ...
  readonly originHostname: string | null // This key is indexed as well.,
  readonly meta: {
    ...
    readonly abi?: string,
    readonly platform?: 'Android' | 'Windows' | // X11 is used for historic reasons, but this value means that it is a Unix platform.
    'Macintosh' | 'X11' | string,
    ...
    readonly toolkit?: 'gtk' | 'gtk3' | 'windows' | 'cocoa' | 'android' | string,
    readonly updateChannel?: // Local builds
    'default' | 'nightly' | // Nightly try builds for QA
    'nightly-try' | // Developer Edition channel
    'aurora' | 'beta' | 'release' | // Extended Support Release channel
    'esr' | string,
...
async function reallyOpen(): Promise<Database> {
  const db = await openDB(DATABASE_NAME, DATABASE_VERSION, {
    upgrade(db: .idb<ObjectStore>, oldVersion: number, newVersion: null | number, transaction: .idb<ObjectStore, null>) {
 ...
 */
export async function persistUploadedProfileInformationToDb(uploadedProfileInformation: UploadedProfileInformation): Promise<void> {
  const db = await open();
 ...
 */
export async function listAllUploadedProfileInformationFromDb(): Promise<UploadedProfileInformation[]> {
  const db = await open();
 ...
 * otherwise.
 */
export async function retrieveUploadedProfileInformationFromDb(profileToken: string): Promise<UploadedProfileInformation | null> {
  if (!profileToken) {
 ...
 */
export async function deleteUploadedProfileInformationFromDb(profileToken: string): Promise<void> {
  const db = await open();
 ...
 */
export async function changeStoredProfileNameInDb(profileToken: string, profileName: string): Promise<void> {
