...
export type FrameTable = {
  ...
  line: number | null[],
  column: number | null[],
  optimizations: Record<any, any> | null[],
  length: number
};
...
export type Thread = {
  ...
  processType: 'default' | 'plugin' | 'tab' | 'ipdlunittest' | 'geckomediaplugin' | 'gpu' | 'pdfium' | // Unknown process type:
...
export type ProfileMeta = {
  ...
  'Android' | 'Windows' | // X11 is used for historic reasons, but this value means that it is a Unix platform.
...
type SerializableThread = (Flow.Diff<Thread, {
  stringTable: UniqueStringArray
}>) & {
  stringArray: string[]
};
