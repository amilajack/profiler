...
'url' | // Important, do not put URL or file path information here, as it will not be
...
'string' | // Data that happened at a specific time, relative to the start of
// the profile. e.g. "Label: 15.5s, 20.5ms, 30.5μs"
'duration' | // The following are alternatives to display a time only in a specific
// unit of time.
'time' | // "Label: 5s"
'seconds' | // "Label: 5ms"
'milliseconds' | // "Label: 5μs"
'microseconds' | // "Label: 5ns"
// e.g. "Label: 5.55mb, 5 bytes, 312.5kb"
'nanoseconds' | // This should be a value between 0 and 1.
// "Label: 50%"
'bytes' | // The integer should be used for generic representations of numbers. Do not
// use it for time information.
// "Label: 52, 5,323, 1,234,567"
'percentage' | // The decimal should be used for generic representations of numbers. Do not
// use it for time information.
...
export type MarkerDisplayLocation = 'marker-chart' | // This adds markers to the main marker timeline in the header.
'marker-table' | // In the timeline, this is a section that breaks out markers that are related
// to memory. When memory counters are enabled, this is its own track, otherwise
// it is displayed with the main thread.
'timeline-overview' | // This adds markers to the IPC timeline area in the header.
'timeline-memory' | // This adds markers to the FileIO timeline area in the header.
'timeline-ipc' | // TODO - This is not supported yet.
'timeline-fileio' | 'stack-chart';
...
T extends any> = (Flow.Diff<T, // Remove the cause property.
{
  cause: any
}>) & {
...
export type NetworkStatus = 'STATUS_START' | 'STATUS_STOP' | 'STATUS_REDIRECT' | 'STATUS_CANCEL';
...
export type IPCMarkerPayload = (IPCMarkerPayload_Gecko) & {
 ...
 * profiler.
 */
export type MarkerPayload = FileIoPayload | GPUMarkerPayload | NetworkPayload | UserTimingMarkerPayload | TextMarkerPayload | LogMarkerPayload | PaintProfilerMarkerTracing | CcMarkerTracing | DOMEventMarkerPayload | GCMinorMarkerPayload | GCMajorMarkerPayload | GCSliceMarkerPayload | StyleMarkerPayload | BHRMarkerPayload | LongTaskMarkerPayload | VsyncTimestampPayload | ScreenshotPayload | NavigationMarkerPayload | PrefMarkerPayload | IPCMarkerPayload | ChromeCompleteTraceEventPayload | ChromeDurationTraceEventPayload | ChromeInstantTraceEventPayload | MediaSampleMarkerPayload | JankPayload | null;

export type MarkerPayload_Gecko = GPUMarkerPayload | NetworkPayload | UserTimingMarkerPayload | LogMarkerPayload | DOMEventMarkerPayload | GCMinorMarkerPayload | GCMajorMarkerPayload_Gecko | GCSliceMarkerPayload_Gecko | VsyncTimestampPayload | ScreenshotPayload | CcMarkerTracing | ArbitraryEventTracing | NavigationMarkerPayload | JsAllocationPayload_Gecko | NativeAllocationPayload_Gecko | PrefMarkerPayload | IPCMarkerPayload_Gecko | // The following payloads come in with a stack property. During the profile processing
// the "stack" property is are converted into a "cause". See the CauseBacktrace type
// for more information.
MediaSampleMarkerPayload | $ReplaceCauseWithStack<FileIoPayload> | $ReplaceCauseWithStack<PaintProfilerMarkerTracing> | $ReplaceCauseWithStack<StyleMarkerPayload> | // Payloads can be null.
$ReplaceCauseWithStack<TextMarkerPayload> | null;
