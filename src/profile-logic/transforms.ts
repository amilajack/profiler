...
import {encodeUintArrayForUrlComponent, decodeUintArrayFromUrlComponent} from '../utils/uintarray-encoding';
import {
 ...
 */
export function getTransformLabelL10nIds(thread: Thread, threadName: string, transforms: Transform[]): Array<TransformLabeL10nIds> {
...
function _collapseDirectRecursionInCallNodePath(
  ...
  const newPath: Array<IndexIntoFuncTable> = [];
  let previousFunc;
...
export function invertCallNodePath(path: CallNodePath, callTree: CallTree, callNodeTable: CallNodeTable): CallNodePath {
  ...
  const pathToLeaf: Array<IndexIntoCallNodeTable | any> = [];
  do {
...
export function mergeCallNode(
  ...
  return timeCode('mergeCallNode', () => {
    ...
    const stackMatches: Array<boolean> = [];
...
export function dropFunction(
  ...
  const stackContainsFunc: Array<undefined | true> = [];
  for (let stackIndex = 0; stackIndex < stackTable.length; stackIndex++) {
    const prefix = stackTable.prefix[stackIndex];
    const frameIndex = stackTable.frame[stackIndex];
    const funcIndex = frameTable.func[frameIndex];
...
export function collapseResource(
  ...
  IndexIntoStackTable | null> = new Map();
...
export function focusInvertedSubtree(
  ...
  return timeCode('focusInvertedSubtree', () => {
    ...
    const { stackTable, frameTable } = thread;
    const funcMatchesImplementation = FUNC_MATCHES[implementation];
    function convertStack(leaf: null | IndexIntoStackTable) {
...
export function restoreAllFunctionsInCallNodePath(
  ...
  const matchesUpToDepth: Array<null | number> = [];
  ...
  const newCallNodePath: Array<IndexIntoFuncTable> = [];
  for (
