...
export function correlateIPCMarkers(threads: Thread[]): IPCMarkerCorrelations {
  ...
  function makeIPCMessageID(thread: Thread, data): string {
    let pids;
  ...
  const markersByKey: Map<string, Array<{
    tid: number,
    index: number,
...
export function deriveMarkersFromRawMarkerTable(
  ...
  function mergeIntervalData(startData: MarkerPayload, endData: MarkerPayload): MarkerPayload {
 ...
 * is present.
 */
export function getAllowMarkersWithNoSchema(markerSchemaByName: MarkerSchemaByName): (arg1: Marker) => boolean | undefined {
...
export function groupScreenshotsById(getMarker: (arg1: MarkerIndex) => Marker, markerIndexes: MarkerIndex[]): Map<string, Marker[]> {
 ...
 */
export function sanitizeFileIOMarkerFilenamePath(payload: FileIoPayload): FileIoPayload {
  if (!payload.filename) {
    return payload;
  }
 ...
 */
export function computeScreenshotSize(payload: ScreenshotPayload, maximumSize: number): {
