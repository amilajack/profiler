...
export function initializeLocalTrackOrderByPid(
  ...
  if (legacyThreadOrder === null) {
    ...
    for (const [pid, tracks] of localTracksByPid) {
      const trackOrder: Array<TrackIndex> = [];
 ...
 */
export function addEventDelayTracksForThreads(threads: Thread[], localTracksByPid: Map<Pid, LocalTrack[]>): Map<Pid, LocalTrack[]> {
 ...
 * localTracksByPid map.
 */
export function addProcessCPUTracksForProcess(counters: Counter[] | null, localTracksByPid: Map<Pid, LocalTrack[]>): Map<Pid, LocalTrack[]> {
  if (counters === null) {
...
// order.
function getDefaultSelectedThreadIndexes(visibleThreadIndexes: ThreadIndex[], profile: Profile): Set<ThreadIndex> {
...
export function tryInitializeHiddenTracksFromUrl(
  ...
  const hiddenTracks = { hiddenGlobalTracks, hiddenLocalTracksByPid } as const;
...
export function computeDefaultHiddenTracks(tracksWithOrder: TracksWithOrder, profile: Profile): HiddenTracks {
  return _computeHiddenTracksForVisibleThreads(
    profile,
...
export function getVisibleThreads(
  ...
  const visibleThreads: Array<ThreadIndex> = [];
  for (
...
export function computeDefaultVisibleThreads(profile: Profile): Set<ThreadIndex> {
...
function _computeThreadDefaultVisibilityScore(profile: Profile, thread: Thread, maxCpuDeltaPerInterval: number | null): DefaultVisibilityScore {
