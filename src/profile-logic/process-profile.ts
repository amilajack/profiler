...
type ExtractionInfo = {
  ...
  stringToNewFuncIndexAndFrameAddress: Map<string, {
...
export function extractFuncsAndResourcesFromFrameLocations(
  ...
  const frameFuncs: Array<IndexIntoFuncTable> = [];
  const frameAddresses: Array<Address | null> = [];
...
function _processProfilerOverhead(
  ...
  const geckoProfilerOverhead: GeckoProfilerOverhead | null | undefined =
 ...
 */
export function adjustTableTimestamps<Table extends {
  time: Milliseconds[]
 ...
 * into milliseconds.
 */
export function adjustProfilerOverheadTimestamps<Table extends {
  time: Microseconds[]
}>(table: Table, delta: Milliseconds): Table {
  return {
    ...table,
...
export function processGeckoOrDevToolsProfile(json: unknown): Profile {
  ...
  const geckoProfile = coerce<unknown, GeckoProfile>(json.profile ? json.profile : json);
...
export function processGeckoProfile(geckoProfile: GeckoProfile): Profile {
  ...
  const profilerOverhead: ProfilerOverhead[] = nullableProfilerOverhead.reduce<Array<any>>((acc, overhead) => {
    if (overhead !== null) {
      acc.push(overhead);
    }
    return acc;
