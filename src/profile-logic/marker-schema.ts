 ...
 */
export function getMarkerSchemaName(markerSchemaByName: MarkerSchemaByName, marker: Marker): string {
 ...
 */
export function getSchemaFromMarker(markerSchemaByName: MarkerSchemaByName, marker: Marker): MarkerSchema | null {
  return (
 ...
 */
export function parseLabel(markerSchema: MarkerSchema, categories: CategoryList, label: string): (arg1: Marker) => string {
  ...
  const computeLabelParts: Array<(arg1: Marker) => string> = splits.map((part, i) => {
    ...
    if (keys.length === 2) {
      ...
      switch (markerKey) {
        case 'start':
          return (marker: Marker) => formatTimestamp(marker.start);
        case 'end':
        ...
        case 'name':
          return (marker: Marker) => marker.name;
        case 'category':
          return (marker: Marker) => categories[marker.category].name;
        case 'data':
        default:
          return parseError(label, part);
      }
