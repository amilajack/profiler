...
function getProfileMeta(profile: unknown): MixedObject {
  if (
    profile &&
  ...
  [10]: (profile) => {
    ...
    function convertToVersionTenRecursive(p: any) {
      for (const thread of p.threads) {
        ...
        const extraMarkers: Array<any | Array<any>> = [];
    ...
    function convertToVersion20Recursive(p: any) {
      for (const thread of p.threads) {
        ...
        for (
          ...
          const newTuple: Array<null | Payload | number | 0 | 1 | 2 | 3> = [];
