...
export type SymbolTableAsTuple = [// addrs
Uint32Array, // index
Uint32Array, // buffer
Uint8Array];
...
export default class SymbolStoreDB {
  ...
  _setupDB(dbName: string): Promise<IDBDatabase> {
    const indexedDB: IDBFactory | undefined = window.indexedDB;
    if (!indexedDB) {
      throw new Error('Could not find indexedDB on the window object.');
    }
    return new Promise((resolve: (result: Promise<IDBDatabase> | IDBDatabase) => void, reject: (error?: any) => void) => {
  ...
  storeSymbolTable(
    debugName: string,
    breakpadId: string,
    [addrs, index, buffer]: [any, any, any],
  ): Promise<void> {
  ...
  getSymbolTable(debugName: string, breakpadId: string): Promise<SymbolTableAsTuple> {
    return this._getDB().then((db) => {
      return new Promise((resolve: (
        result: Promise<Array<Uint8Array | Uint32Array>> | Array<Uint8Array | Uint32Array>,
  ...
  _deleteRecordsLastUsedBeforeDate(store: SymbolStore, beforeDate: Date, callback: () => void): void {
    ...
    const cursorReq = lastUsedDateIndex.openCursor(
      (range as IDBKeyRange<SymbolDateKey>)
    );
  ...
  _count(store: SymbolStore, callback: (arg1: number) => void): void {
