 ...
 */
export function convertPerfScriptProfile(profile: string): GeckoProfileVersion24 {
  function _createThread(name: string, pid: number, tid: number) {
    ...
    const stringTable: Array<any | string> = [];

    const stackMap = new Map();
    function getOrCreateStack(frame: number, prefix: null | number) {
    ...
    function addSample(threadName: string, stackArray: Array<any | string>, time: number) {
  ...
  function _addThreadSample(pid: number, tid: number, threadName: string, timeStamp: number, stack: Array<any | string>) {
