...
export type TracingEventUnion = ProfileEvent | ProfileChunkEvent | CpuProfileEvent | ThreadNameEvent | ProcessNameEvent | ProcessLabelsEvent | ProcessSortIndexEvent | ThreadSortIndexEvent | ScreenshotEvent;
...
export function attemptToConvertChromeProfile(json: unknown): Promise<Profile> | null {
  if (!json) {
    return null;
  }
  ...
  if (Array.isArray(json)) {
    ...
    if (
      ...
      events = coerce<unknown[], TracingEventUnion[]>(json);
    }
  } else if (
...
type ThreadInfo = {
  ...
  nodeIdToStackId: Map<number | undefined, IndexIntoStackTable | null>,
...
function getTimeDeltas(event: ProfileChunkEvent | CpuProfileEvent): number[] | undefined {
  switch (event.name) {
    case 'ProfileChunk':
...
function makeFunctionInfoFinder(categories: CategoryList) {
  ...
  return function getFunctionInfo(functionName: string, hasURLOrLineNumber: boolean): FunctionInfo {
    switch (functionName) {
      case '(idle)':
...
async function extractScreenshots(
  ...
  eventsByName: Map<string, TracingEventUnion[]>,
  profile: Profile,
  screenshots?: ScreenshotEvent[] | null,
): Promise<void> {
  if (!screenshots) {
    return;
  }
 ...
 */
function getImageSize(url: string): Promise<null | {
  width: number,
  height: number
}> {
