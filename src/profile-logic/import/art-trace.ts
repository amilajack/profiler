...
function detectArtTraceFormat(traceBuffer: ArrayBuffer): 'regular' | 'streaming' | 'unrecognized' {
  try {
...
function parseSummary(reader: ByteReader) {
  ...
  const summaryDetails = { summaryVersion, clock: 'thread-cpu' } as const;
  while (true) {
    line = reader.getLine();
...
function parseThreads(reader: ByteReader) {
  ...
  const threads: Array<ArtTraceThread> = [];
  let line = '';
  while (true) {
    line = reader.getLine();
    if (!line || line.startsWith('*')) {
...
function parseMethods(reader: ByteReader) {
  ...
  const methods: Array<ArtTraceMethod> = [];
  let line = '';
  while (true) {
    line = reader.getLine();
    if (!line || line.startsWith('*')) {
...
function parseRecord(reader: ByteReader, version: number, recordSize: number, clock: string) {
...
function parseStreamingFormat(reader: ByteReader) {
  ...
  const methods: Array<ArtTraceMethod> = [];
  const threads: Array<ArtTraceThread> = [];
  const recordStartPositions: Array<any | number> = [];
...
export function getSpecialCategory(methods: ArtTraceMethod[]): SpecialCategoryInfo | undefined {
  ...
  significantSegmentCounts.sort(([_s1, c1]: [any, any], [_s2, c2]: [any, any]) => c2 - c1);
...
export class CategoryInfo {
  ...
  _specialCategoryInfo: SpecialCategoryInfo | undefined;
...
class ThreadBuilder {
  ...
  _getOrCreateStack(frame: number, prefix) {
...
export function convertArtTraceProfile(traceBuffer: ArrayBuffer): GeckoProfileVersion11 {
