...
interface SymbolProvider {
  ...
  requestSymbolsFromServer(requests: LibSymbolicationRequest[]): Promise<LibSymbolicationResponse[]>;
  // Expensive, should be called if requestSymbolsFromServer was unsuccessful.
  requestSymbolsFromBrowser(requests: LibSymbolicationRequest[]): Promise<LibSymbolicationResponse[]>;
...
export interface AbstractSymbolStore {
  getSymbols(
    ...
    successCb: (arg1: RequestedLib, arg2: Map<number, AddressResult>) => void,
    errorCb: (arg1: LibSymbolicationRequest, arg2: Error) => void,
  ): Promise<void>;
}
...
export function readSymbolsFromSymbolTable(
  ...
  demangleCallback: (arg1: string) => string,
...
type DemangleFunction = (arg1: string) => string;
...
export class SymbolStore {
  ...
  async getSymbols(
    ...
    // was successful.
    const requestsForNonCachedLibs: Array<LibSymbolicationRequest> = [];
    const resultsForCachedLibs: Array<any | {
      addresses: Set<number>,
      lib: RequestedLib,
      symbolTable: SymbolTableAsTuple
    }> = [];
    ...
    const symbolicationApiRequestsAndResponsesPerChunk: Array<[LibSymbolicationRequest[], Promise<LibSymbolicationResponse[]>]> = chunks.map((requests) => [
      requests,
    ...
    await Promise.all(
      symbolicationApiRequestsAndResponsesPerChunk.map(
        async ([option2Requests, option2ResponsePromise]: [any, any]) => {
  ...
  async _processSuccessfulResponsesAndReturnRemaining(
    ...
    try {
      ...
      const failedRequests: Array<LibSymbolicationRequest> = [];
