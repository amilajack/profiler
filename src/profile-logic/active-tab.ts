 ...
 */
export function computeActiveTabTracks(profile: Profile, relevantPages: Page[], state: State): ActiveTabTimeline {
  ...
  const mainTrackIndexes: Array<any | number> = [];
  const resources: Array<ActiveTabResourceTrack> = [];
  const screenshots: Array<ActiveTabScreenshotTrack> = [];
...
function getTopmostInnerWindowIDs(relevantPages: Page[]): Set<InnerWindowID> {
  const topmostInnerWindowIDs: Array<InnerWindowID | any> = [];
 ...
 */
function _getActiveTabResourceName(thread: Thread, innerWindowIDToPageMap: Map<InnerWindowID, Page>): string {
  if (isMainThread(thread)) {
    // This is a sub-frame.
