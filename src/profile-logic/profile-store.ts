...
export function uploadBinaryProfileData() {
  ...
  return {
    ...
    startUpload: (data: $TypedArray, progressChangeCallback?: (arg1: number) => unknown): Promise<string> => new Promise((resolve: (result: Promise<string> | string) => void, reject: (error?: any) => void) => {
      if (isAborted) {
