...
export function getJsTracerLeafTiming(jsTracer: JsTracerTable, stringTable: UniqueStringArray): JsTracerTiming[] {
  ...
  const isUrlCache: Array<boolean> = [];
  const isUrlRegex = /:\/\//;
...
export function convertJsTracerToThreadWithoutSamples(fromThread: Thread, jsTracer: JsTracerFixed, categories: CategoryList): {
  ...
  const funcTable = { ...fromThread.funcTable } as const;
...
export function getSelfTimeSamplesFromJsTracer(
  ...
  const epsilon = 1e-5;
  const isNearlyEqual = (a: Microseconds, b: Microseconds) => Math.abs(a - b) < epsilon;
  // Each event type will have it's own timing information, later collapse these into
  // a single array.
  const { stringTable } = thread;
  const samples = getEmptySamplesTableWithEventDelay();
  const sampleWeights: Array<number> = [];
