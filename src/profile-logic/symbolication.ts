...
export type SymbolicationStepCallback = (threadIndex: ThreadIndex, symbolicationStepInfo: SymbolicationStepInfo) => void;
 ...
 */
function makeConsensusMap<K, V>(iterableOfEntryPairs: Iterable<[K, V]>): Map<K, V> {
 ...
 */
function getThreadSymbolicationInfo(thread: Thread, libs: Lib[]): ThreadSymbolicationInfo {
  ...
  for (
    ...
    const allFramesForThisLib: Array<IndexIntoFrameTable> = [];
    const frameAddresses: Array<Address> = [];
...
function buildLibSymbolicationRequestsForAllThreads(symbolicationInfo: ThreadSymbolicationInfo[]): LibSymbolicationRequest[] {
  ...
  return Array.from(libKeyToAddressesMap).map(([libKey, addresses]: [any, any]) => {
    const [debugName, breakpadId] = libKey.split('/');
    const lib = { debugName, breakpadId } as const;
...
function finishSymbolicationForLib(
  ...
  for (let threadIndex = 0; threadIndex < threads.length; threadIndex++) {
    ...
    const symbolicationStep = { threadLibSymbolicationInfo, resultsForLib } as const;
...
function _removeFramesFromStackTable(stackTable: StackTable, framesToRemove: Set<IndexIntoFrameTable>): {
  stackTable: StackTable,
  oldStackToNewStack: Map<IndexIntoStackTable, IndexIntoStackTable | null> | null
} {
  if (framesToRemove.size === 0) {
    return { stackTable, oldStackToNewStack: null };
  }
  const newStackTable = getEmptyStackTable();
  const oldStackToNewStack: Map<IndexIntoStackTable, IndexIntoStackTable | null> = new Map();
...
function _addExpansionStacksToStackTable(
  stackTable: StackTable,
  frameIndexToInlineExpansionFrames: Map<IndexIntoFrameTable, IndexIntoFrameTable[]>,
): {
  stackTable: StackTable,
  oldStackToNewStack: Map<IndexIntoStackTable, IndexIntoStackTable | null> | null
} {
...
export function applySymbolicationStep(
  ...
  const symbolAddressToCanonicalSymbolIndexMap: Map<Address, IndexIntoNativeSymbolTable> = new Map();
  ...
  for (const frameIndex of nonInlinedFrames) {
    ...
    let addressResult: AddressResult | undefined = resultsForLib.get(address);
  ...
  const frameIndexToInlineExpansionFrames: Map<IndexIntoFrameTable, IndexIntoFrameTable[]> = new Map();
  ...
  for (const frameIndex of nonInlinedFrames) {
    ...
    const inlineExpansionFrames: Array<IndexIntoFrameTable> = [];
    const inlineExpansionFuncIndexes: Array<IndexIntoFuncTable | any | number> = [];
    for (
      let inlineDepth = 0;
...
// result: [1, 4, 1, 3]
export function applyFuncSubstitutionToCallPath(oldFuncToNewFuncsMap: FuncToFuncsMap, path: CallNodePath): CallNodePath {
...
export function applyFuncSubstitutionToPathSetAndIncludeNewAncestors(oldFuncToNewFuncsMap: FuncToFuncsMap, pathSet: PathSet): PathSet {
  const newPathSet: Array<CallNodePath | any | Array<IndexIntoFuncTable>> = [];
