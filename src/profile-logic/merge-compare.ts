 ...
 */
export function mergeProfilesForDiffing(profiles: Profile[], profileStates: UrlState[]): {
  profile: Profile,
  ...
  const implementationFilters: Array<ImplementationFilter> = [];
...
function mergeCategories(categoriesPerThread: Array<CategoryList | undefined>): {
  ...
  const translationMaps: Array<TranslationMapForCategories> = [];
 ...
 * functions.
 */
function combineResourceTables(newStringTable: UniqueStringArray, threads: ReadonlyArray<Thread>): {
  resourceTable: ResourceTable,
  translationMaps: TranslationMapForResources[]
} {
  const mapOfInsertedResources: Map<string, IndexIntoResourceTable> = new Map();
  const translationMaps: Array<TranslationMapForResources> = [];
 ...
 */
function combineNativeSymbolTables(newStringTable: UniqueStringArray, threads: ReadonlyArray<Thread>): {
  ...
  const mapOfInsertedNativeSymbols: Map<string, IndexIntoNativeSymbolTable> =
    new Map();
  const translationMaps: Array<TranslationMapForNativeSymbols> = [];
...
function combineFuncTables(
  ...
  const translationMaps: Array<TranslationMapForFuncs> = [];
...
function combineFrameTables(
  ...
  const translationMaps: Array<TranslationMapForFrames> = [];
  ...
  threads.forEach((thread, threadIndex) => {
    ...
    const getNewCategory = (category: IndexIntoCategoryList) => {
...
function combineStackTables(
  ...
  const translationMaps: Array<TranslationMapForStacks> = [];
 ...
 * subsequent merging functions, if necessary.
 */
function combineSamplesForMerging(translationMapsForStacks: TranslationMapForStacks[], threads: Thread[]): SamplesTable {
...
function mergeMarkers(
  ...
  const translationMaps: Array<TranslationMapForMarkers> = [];
 ...
 */
function mergeScreenshotMarkers(threads: Thread[], targetThread: Thread): {
  markerTable: RawMarkerTable,
  translationMaps: TranslationMapForMarkers[]
} {
