...
export function getTimeRangeIncludingAllThreads(profile: Profile): StartEndRange {
  const completeRange = { start: Infinity, end: -Infinity } as const;
...
function _filterThreadByFunc(
  ...
  return timeCode('filterThread', () => {
    ...
    function convertStack(stackIndex: null | IndexIntoStackTable) {
      if (stackIndex === null) {
        return null;
      }
...
export function filterThreadToSearchString(thread: Thread, searchString: string): Thread {
  ...
  function stackMatchesFilter(stackIndex: null | IndexIntoStackTable) {
    if (stackIndex === null) {
      return false;
    }
...
export function filterThreadByTab(thread: Thread, relevantPages: Set<InnerWindowID>): Thread {
  return timeCode('filterThreadByTab', () => {
    ...
    const frameMatchesFilterCache: Map<IndexIntoFrameTable, boolean> =
      new Map();
    function frameMatchesFilter(frame: IndexIntoFrameTable) {
...
export function accumulateCounterSamples(samplesArray: Array<CounterSamplesTable>): Array<AccumulatedCounterSamples> {
  const accumulatedSamples = samplesArray.map((samples) => {
    ...
    let maxCount = 0;
    let accumulated = 0;
    const accumulatedCounts: Array<number> = [];
 ...
 */
export function computeMaxCounterSampleCounts(samplesArray: Array<CounterSamplesTable>): Array<number> {
...
export function getCallNodePathFromIndex(callNodeIndex: IndexIntoCallNodeTable | null, callNodeTable: CallNodeTable): CallNodePath {
  ...
  const callNodePath: Array<IndexIntoFuncTable> = [];
 ...
 */
export function convertStackToCallNodeAndCategoryPath(thread: Thread, stack: IndexIntoStackTable): CallNodeAndCategoryPath {
  const { stackTable, frameTable } = thread;
  const path: Array<CallNodeAndCategory> = [];
  for (
...
export function invertCallstack(thread: Thread, defaultCategory: IndexIntoCategoryList): Thread {
  return timeCode('invertCallstack', () => {
    ...
    function stackFor(prefix: null | number, frame: IndexIntoFrameTable, category: IndexIntoCategoryList, subcategory: IndexIntoSubcategoryListForCategory) {
...
export function getSampleIndexClosestToStartTime(samples: SamplesTable, time: number, interval: Milliseconds): IndexIntoSamplesTable {
...
function _shouldShowBothOriginAndFileName(fileName: string, origin: string, resourceType: resourceTypeEnum | null): boolean {
 ...
 */
export function getFuncNamesAndOriginsForPath(path: CallNodeAndCategoryPath, thread: Thread): Array<{
  funcName: string,
...
export function findBestAncestorCallNode(
  ...
  function limitSameCategoryPathToCommonAncestor(callNode: IndexIntoCallNodeTable | number) {
...
export function filterToRetainedAllocations(nativeAllocations: BalancedNativeAllocationsTable): NativeAllocationsTable {
  ...
  const memoryAddressToAllocation: Map<Address, IndexIntoAllocations> =
    new Map();
  const retainedAllocation: Array<boolean> = [];
  for (
    let allocationIndex = 0;
    allocationIndex < nativeAllocations.length;
    allocationIndex++
...
export function replaceStackReferences(
  ...
  function mapBacktraceSelfStack(oldStackIndex: null | IndexIntoStackTable) {
    if (oldStackIndex === null) {
      return null;
    }
  ...
  // Markers
  function replaceStackReferenceInMarkerPayload(oldData: MarkerPayload): MarkerPayload {
