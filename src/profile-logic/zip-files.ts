...
export function createZipTable(zipEntries: JSZip): ZipFileTable {
  const fullPaths = objectEntries(zipEntries.files)
    .filter(([_fileName, file]: [any, any]) => !file.dir)
    .map(([fileName, _file]: [any, any]) => fileName);
...
export class ZipFileTree {
  ...
  _computeChildrenArray(parentIndex: IndexIntoZipFileTable | null): IndexIntoZipFileTable[] {
    const children: Array<IndexIntoZipFileTable> = [];
    for (
      let childIndex = 0;
  ...
  getAllDescendants(zipTableIndex: IndexIntoZipFileTable | null): Set<IndexIntoZipFileTable> {
