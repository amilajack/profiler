...
export function sanitizePII(
  ...
  const newProfile = {
    ...
    pages: pages,
    threads: profile.threads.reduce<Array<any>>((acc, thread, threadIndex) => {
    ...
    counters: profile.counters
      ? profile.counters.reduce<Array<any>>((acc, counter) => {
    ...
    profilerOverhead: profile.profilerOverhead
      ? profile.profilerOverhead.reduce<Array<any>>((acc, overhead) => {
          const newThreadIndex = oldThreadIndexToNew.get(
...
function sanitizeThreadPII(
  ...
  if (
    ...
    const sanitizedFuncIndexesToFrameIndex: Map<IndexIntoFuncTable, IndexIntoFrameTable[]> = new Map();
