...
export const getFullProfileSpecificState: Selector<FullProfileSpecificUrlState> = (state) => getProfileSpecificState(state).full;
export const getActiveTabProfileSpecificState: Selector<ActiveTabSpecificProfileUrlState> = (state) => getProfileSpecificState(state).activeTab;
...
export const getTimelineTrackOrganization: Selector<TimelineTrackOrganization> = (state) => getUrlState(state).timelineTrackOrganization;
...
export const getSelectedThreadIndexesOrNull: Selector<Set<ThreadIndex> | null> = (state) => getProfileSpecificState(state).selectedThreads;
 ...
 * checks everywhere.
 */
export const getHiddenLocalTracks: DangerousSelectorWithArguments<Set<TrackIndex>, Pid> = (state, pid) =>
  ensureExists(
 ...
 */
export const getLocalTrackOrder: DangerousSelectorWithArguments<TrackIndex[], Pid> = (state, pid) =>
  ensureExists(
...
const EMPTY_TRANSFORM_STACK: Array<never> = [];

export const getTransformStack: DangerousSelectorWithArguments<TransformStack, ThreadsKey> = (state, threadsKey) => {
  return (
 ...
 */
export const getUrlPredictor: Selector<(arg1: Action | Action[]) => string> =
  createSelector(
    getUrlState,
