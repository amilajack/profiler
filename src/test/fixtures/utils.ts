...
import {getCallTree, computeCallTreeCountsAndSummary, CallTree} from 'firefox-profiler/profile-logic/call-tree';
 ...
 */
export function callTreeFromProfile(profile: Profile, threadIndex: number = 0): CallTree {
 ...
 * wait on.
 */
export function waitUntilState(store: Store, predicate: (arg1: State) => boolean): Promise<void> {
 ...
 */
export async function waitUntilData<T>(predicate: () => Promise<T> | T, times: number = 10): Promise<T> {
  function wait() {
    return new Promise((resolve: (result: Promise<undefined> | undefined) => void) => setTimeout(resolve, 50));
  }
 ...
 */
export function findFillTextPositionFromDrawLog(drawLog: any[], fillText: string): {
  x: number,
  y: number
} {
  const positions = drawLog
    .filter(([cmd, text]: [any, any]) => cmd === 'fillText' && text === fillText)
    .map(([,, x, y]: [any, any, any, any]) => ({ x, y }));
