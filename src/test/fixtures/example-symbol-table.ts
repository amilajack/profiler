...
export type ExampleSymbolTable = {
  ...
  getAddressResult: (arg1: number) => AddressResult | null
};
...
function _getInlinesRecursive(inlinedCall: InlinedCall | null | undefined, address: number): AddressInlineFrame[] {
  if (!inlinedCall) {
    return [];
  }
...
// This function is part of the simulated symbolication API for testing.
function _getLineAndInlines(lineRanges: LineRange[] | null | undefined, address: number): {
  line?: number,
  inlines?: AddressInlineFrame[]
} {
  if (!lineRanges) {
    return {};
  }
...
function _makeGetAddressResultFunction(syms: ExampleSymbolTableSymbols): (arg1: number) => AddressResult | null {
