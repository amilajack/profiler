...
// Instant marker:
// No payload:
[MarkerName, MarkerTime] | [MarkerName, // start time
MarkerTime, // end time
MarkerTime | null] | [MarkerName, // start time
MarkerTime, // end time
MarkerTime | null, MarkerPayload | MockPayload]>;
...
export function addMarkersToThreadWithCorrespondingSamples(
  ...
  markers.forEach((tuple) => {
    ...
    const payload: MarkerPayload = (tuple as any)[3] || null;
 ...
 */
export function makeStartMarker(name: string, startTime: Milliseconds): TestDefinedRawMarker {
  return {
    name,
    startTime,
    endTime: null,
 ...
 * A utility to make TestDefinedRawMarker
 */
export function makeEndMarker(name: string, endTime: Milliseconds): TestDefinedRawMarker {
  return {
    name,
    startTime: null,
    endTime,
 ...
 * A utility to make TestDefinedRawMarker
 */
export function makeInstantMarker(name: string, startTime: Milliseconds): TestDefinedRawMarker {
  return {
    name,
    startTime,
    endTime: null,
 ...
 * A utility to make TestDefinedRawMarker
 */
export function makeIntervalMarker(name: string, startTime: Milliseconds, endTime: Milliseconds): TestDefinedRawMarker {
  return {
    name,
    startTime,
    endTime,
 ...
 * A utility to make TestDefinedRawMarker
 */
export function makeCompositorScreenshot(startTime: Milliseconds): TestDefinedRawMarker {
  return {
...
export function getProfileFromTextSamples(...allTextSamples: string[]): {
  ...
  const funcNamesPerThread: Array<Array<string>> = [];
  const funcNamesDictPerThread: Array<{
    [funcName: string]: number
  }> = [];
  ...
  profile.threads = allTextSamples.map((textSamples) => {
    ...
    const funcNamesDict = funcNames.reduce<Record<string, any>>((result, item, index) => {
      result[item] = index;
      return result;
    }, {});
...
function _getAllMatchRanges(regex: RegExp, str: string): Array<{
  start: number,
  end: number
}> {
  const ranges: Array<{
...
function _findFileNameFromFuncName(funcNameWithModifier: string): string | null {
...
function _findLineNumberFromFuncName(funcNameWithModifier: string): LineNumber | null {
...
function _findAddressFromFuncName(funcNameWithModifier: string): Address | null {
...
function _findInlineDepthFromFuncName(funcNameWithModifier: string): Address | null {
...
function _buildThreadFromTextOnlyStacks(
  ...
  const resourceIndexCache: Record<string, any> = {};
...
function _getIPCMarkers(options: Partial<IPCMarkersOptions> = {}): TestDefinedMarkers {
...
export function getScreenshotTrackProfile() {
  const screenshotMarkersForWindowId = (windowID: string) =>
    Array(10)
      .fill()
      .map((_, i) => [
 ...
 */
export function getProfileWithEventDelays(userEventDelay?: Milliseconds[]): Profile {
  const profile = getEmptyProfile();
  profile.meta.markerSchema = markerSchemaForTests;
 ...
 */
export function getThreadWithEventDelay(userEventDelay?: Milliseconds[]): Thread {
  const thread = getEmptyThread();
...
export function addInnerWindowIdToStacks(
  thread: Thread,
  listOfOperations: Array<{
    innerWindowID: number,
    callNodes: CallNodePath[]
  }>,
 ...
 */
export function getThreadWithThreadCPUDelta(userThreadCPUDelta?: Array<number | null>, interval: Milliseconds = 1): Thread {
  const thread = getEmptyThread();
