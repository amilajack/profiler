...
export function createGeckoSubprocessProfile(parentProfile: GeckoProfile, extraMarkers: GeckoMarkerTuple[] = []): GeckoSubprocessProfile {
...
type TestDefinedGeckoMarker = {
  ...
  readonly data?: MarkerPayload_Gecko
};

function _createGeckoThreadWithMarkers(markers: TestDefinedGeckoMarker[]): GeckoThread {
 ...
 */
export function createGeckoProfileWithMarkers(markers: TestDefinedGeckoMarker[]): GeckoProfile {
...
export function createGeckoProfilerOverhead(thread: GeckoThread): GeckoProfilerOverhead {
  ...
  const data: Array<[Nanoseconds, Nanoseconds, Nanoseconds, Nanoseconds, Nanoseconds]> = [];
  for (let i = 0; i < thread.samples.data.length; i++) {
    // Go through all the thread samples and create a corresponding counter entry.
