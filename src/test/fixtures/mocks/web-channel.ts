...
import type {ResponseFromBrowser, MessageFromBrowser, MessageToBrowser} from '../../../app-logic/web-channel';
...
export function mockWebChannel() {
  const messagesSentToBrowser: Array<any | any> = [];
  ...
  function triggerResponse<R extends ResponseFromBrowser>(message: MessageFromBrowser<R>) {
  ...
  function registerMessageToChromeListener(listener: (arg1: MessageToBrowser) => void): void {
...
export function simulateWebChannel(profileGetter: () => unknown) {
  const webChannel = mockWebChannel();
