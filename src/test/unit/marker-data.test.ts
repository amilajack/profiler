...
import {getThreadSelectors, selectedThreadSelectors} from 'firefox-profiler/selectors';
import {
  INSTANT,
  INTERVAL,
  INTERVAL_START,
...
describe('Derive markers from Gecko phase markers', function () {
  function setupWithTestDefinedMarkers(markers: any) {
  ...
  it('has special handling for CompositorScreenshot', function () {
    ...
    const payloadsForWindowB = payloadsForWindowA.map((payload: any) => ({
      ...payload,
...
describe('deriveMarkersFromRawMarkerTable', function () {
  ...
  it('creates 17 markers given the test data', function () {
    ...
    const markerNames = markers.map(
      (marker: any) => (marker.data ? marker.data.type : 'null') + ':' + marker.name
    );
...
describe('filterRawMarkerTableToRange', () => {
  ...
  function setup({
    ...
    const rawMarkerNames = rawMarkerTable.name.map((i: any) =>
    ...
    const processedMarkerNames = processedMarkers.map(({
      name,
    }: any) => name);
  ...
  it('filters network markers', () => {
    ...
    expect(
      processedMarkers.map((marker: any) => [
        marker.name,
  ...
  it('filters network markers with only a start marker', () => {
    ...
    const result = processedMarkers.map((marker: any) => [
      marker.name,
      marker.data && (marker.data as any).id,
    ]);
...
describe('filterRawMarkerTableToRangeWithMarkersToDelete', () => {
  ...
  function setup({
    ...
    const markerNames = rawMarkerTable.name.map((stringIndex: any) =>
