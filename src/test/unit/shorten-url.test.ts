...
function mockFetchForBitly({
  ...
  endpointUrl: string,
  responseFromRequestPayload: (arg1?: any) => any
}) {
  window.fetch
    .catch(404) // catch all
    .mock(endpointUrl, async (urlString: any, options: any) => {
...
describe('shortenUrl', () => {
  function mockFetchWith(returnedHash: any) {
    mockFetchForBitly({
...
describe('expandUrl', () => {
  function mockFetchWith(returnedLongUrl: any) {
    mockFetchForBitly({
