...
describe('SymbolStore', function () {
  let symbolProvider: any, symbolStore: any;
  ...
  it('should only request symbols from the symbol provider once per library', async function () {
    symbolProvider = {
      requestSymbolsFromServer: jest.fn((requests: any) =>
        Promise.resolve(
      ...
      requestSymbolsFromBrowser: async (_path: any, _requestJson: any) => {
    ...
    const lib1 = { debugName: 'firefox', breakpadId: 'dont-care' } as const;
    let secondAndThirdSymbol = new Map();
    const errorCallback = jest.fn((_request: any, _error: any) => {});
    await symbolStore.getSymbols(
      [{ lib: lib1, addresses: new Set([0xf01, 0x1a50]) }],
      (request: any, results: any) => {
    ...
    const lib2 = { debugName: 'firefox2', breakpadId: 'dont-care2' } as const;
  ...
  it('should persist in DB', async function () {
    symbolProvider = {
      ...
      requestSymbolsFromBrowser: jest.fn((_path: any, _requestJson: any) =>
        Promise.reject(
    ...
    const lib = { debugName: 'firefox', breakpadId: 'dont-care' } as const;
    const errorCallback = jest.fn((_request: any, _error: any) => {});
    await symbolStore.getSymbols(
  ...
  it('should call requestSymbolsFromServer first', async function () {
    ...
    symbolProvider = {
      requestSymbolsFromServer: jest.fn(async (requests: any) => {
  ...
  it('should should report the right errors', async function () {
    ...
    symbolProvider = {
      requestSymbolsFromServer: async (requests: any) => {
        const responses: Array<any> = [];
        for (const request of requests) {
    ...
    await symbolStore.getSymbols(
      libs.map((lib: any) => ({ lib, addresses })),
      (lib: any, _results: any) => {
