...
import {formatFromMarkerSchema, parseLabel, markerSchemaFrontEndOnly} from '../../profile-logic/marker-schema';
...
describe('getMarkerSchema', function () {
  it('combines front-end and Gecko marker schema', function () {
    ...
    expect(
      profile.meta.markerSchema.find((schema: any) => schema.name === 'Jank')
    ).toBeUndefined();
    expect(
      markerSchemaFrontEndOnly.find((schema: any) => schema.name === 'Jank')
    ).toBeTruthy();
    expect(
      combinedSchema.find((schema: any) => schema.name === 'Jank')
    ).toBeTruthy();

    // Find the Gecko only marker schema.
    expect(
      profile.meta.markerSchema.find((schema: any) => schema.name === 'GCMajor')
    ).toBeTruthy();
    expect(
      markerSchemaFrontEndOnly.find((schema: any) => schema.name === 'GCMajor')
    ).toBeUndefined();
    expect(
      combinedSchema.find((schema: any) => schema.name === 'GCMajor')
    ).toBeTruthy();
  });
});
