...
describe('extract functions and resource from location strings', function () {
  ...
  const locationIndexes = locations.map((location: any) =>
  ...
  it('extracts the information for all different types of locations', function () {
    ...
    expect(
      frameFuncs.map((funcIndex: any, locationIndex: any) => {
        ...
        let libIndex: any, resourceName: any, host: any, resourceType: any;
        if (resourceIndex === -1) {
          resourceName = null;
          host = null;
          resourceType = null;
...
describe('gecko counters processing', function () {
  function setup() {
    ...
    const findMainThread = (profile: any) =>
      ensureExists(
        profile.threads.find((thread: any) => thread.name === 'GeckoMain'),
  ...
  it('can extract the counter information correctly', function () {
    ...
    const findMainThreadIndexByPid = (pid: number): number => processedProfile.threads.findIndex(
      (thread: any) => thread.name === 'GeckoMain' && thread.pid === pid
    );
  ...
  it('offsets the counter timing for child processes', function () {
    ...
    const offsetTime = originalTime.map((n: any) => n + 1000);

    const extractTime = (counter: any) => {
      if (counter.sample_groups.length === 0) {
        return [];
      }
      return counter.sample_groups[0].samples.data.map((tuple: any) => tuple[0]);
    };
...
describe('gecko profilerOverhead processing', function () {
  ...
  it('offsets the overhead timing for child processes', function () {
    ...
    const processedTime = originalTime.map((n: any) => n / 1000);
    const offsetTime = processedTime.map((n: any) => n + 1000);

    const extractTime = (overhead: any) =>
      overhead.samples.data.map((tuple: any) => tuple[0]);
...
describe('js allocation processing', function () {
  ...
  function getFrameAddressesForStack(thread: any, stackIndex: any) {
