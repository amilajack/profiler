...
function getSamplesPixelPosition(sampleIndex: IndexIntoSamplesTable, samplePosition: any): CssPixels {
...
describe('TrackMemory', function () {
  function setup() {
    ...
    const moveMouseAtCounter = (index: any, pos: any) =>
      fireEvent(
        canvas,
...
describe('TrackMemory with intersection observer', function () {
  ...
  it('will not draw before the intersection observer', () => {
    ...
    expect(drawCalls.some(([operation]: [any]) => operation === 'clearRect')).toBe(
      false
    );
  });
