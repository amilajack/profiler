...
function setupWithProfile(profile: any) {
  ...
  function fireMouseEvent(eventName: any, options: any) {
    fireEvent(
      ensureExists(
...
describe('MarkerChart', function () {
  ...
  it('does not render several small markers on the same pixel', () => {
    ...
    const fillRectOperations = drawCalls.filter(
      ([operation,,, w, h]: [any, any, any, any, any]) =>
        operation === 'fillRect' &&
  ...
  describe('context menus', () => {
    ...
    function setupForContextMenus() {
      ...
      function clickOnMenuItem(stringOrRegexp: any) {
  ...
  describe('with search strings', function () {
    function getFillTextCalls(drawCalls: any) {
      return drawCalls
        .filter(([methodName]: [any]) => methodName === 'fillText')
    ...
    it('renders lots of markers initially', function () {
      ...
      expect(text.filter((t: any) => t === searchString).length).toBe(1);
    });
  ...
  describe('with active tab', () => {
    ...
    it('does not render the hovered label', () => {
      ...
      const getLabelFromDrawLog = (drawLog: any, markerLabel: any) =>
        drawLog.filter(
          ([operation, text]: [any, any]) =>
