...
function setupWithProfile(profile: any) {
  ...
  function getUrlShorteningParts(): Array<[string, string]> {
    ...
    ).map((node: any) => [node.className, node.textContent]);
  }
  ...
  const getBarElementStyles = () =>
    getBarElements().map((element: any) => element.getAttribute('style'));

  const getPhaseElements = () =>
    Array.from(container.querySelectorAll('.networkChartRowItemBarPhase'));

  const getPhaseElementStyles = () =>
    getPhaseElements().map((element: any) => element.getAttribute('style'));
