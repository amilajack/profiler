...
function simulateSymbolStoreHasNoCache() {
  ...
  (SymbolStoreDB as any).mockImplementation(() => ({
    getSymbolTable: jest
      .fn()
      .mockImplementation((debugName: any, breakpadId: any) =>
        Promise.reject(
...
describe('actions/receive-profile', function () {
   ...
   * the end of func.
   */
  async function observeStoreStateChanges(store: any, func: any) {
  ...
  describe('viewProfile', function () {
    ...
    it('will hide idle content threads', function () {
      ...
      profile.threads.forEach((thread: any, threadIndex: any) => {
    ...
    describe('too many threads', function () {
      it('will limit the visible threads to 15', function () {
        ...
        for (let i = 0; i < profile.threads.length; i++) {
          ...
          ).reduce((accum: any, delta: any) => accum + (delta ?? 0), 0);
  ...
  describe('retrieveProfileFromBrowser', function () {
    ...
    function _setup(profileAs = 'json') {
      ...
      jest.spyOn(console, 'warn').mockImplementation((error: any) => {
  ...
  describe('retrieveProfileFromStore', function () {
    beforeEach(function () {
      ...
      jest
        ...
        .mockImplementation((callback: any) => process.nextTick(callback));
    });
  ...
  describe('retrieveProfileFromFile', function () {
    ...
    async function setupTestWithFile(mockFileOptions: any) {
  ...
  describe('retrieveProfilesToCompare', function () {
    ...
    async function setupWithShortUrl(
      ...
      (expandUrl as any).mockImplementation((shortUrl: any) => {
        switch (shortUrl) {
          case shortUrl1:
            return longUrl1;
          case shortUrl2:
    ...
    async function setup(
      ...
      if (skipMarkers !== true) {
        profile1.threads.forEach((thread: any) =>
        ...
        profile2.threads.forEach((thread: any) =>
          addMarkersToThreadWithCorrespondingSamples(thread, [
            ['F', 1, 3],
            ['G', 2],
            ['H', 3],
  ...
  describe('retrieveProfileForRawUrl', function () {
    async function setup(
      ...
      const waitUntilPhase = (phase: any) =>
        waitUntilState(store, (state: any) => getView(state).phase === phase);
