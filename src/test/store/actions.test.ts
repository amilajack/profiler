...
describe('selectors/getFlameGraphTiming', function () {
   ...
   */
  function getHumanReadableFlameGraphRanges(store: any, funcNames: any) {
   ...
   */
  function getHumanReadableFlameGraphTimings(store: any, funcNames: any) {
    const { callNodeTable } = selectedThreadSelectors.getCallNodeInfo(
      store.getState()
    );
    const flameGraphTiming = selectedThreadSelectors.getFlameGraphTiming(
  ...
  const {
    ...
    funcNamesPerThread: [funcNames],
      ...
      D  G  J  J  J
         H
    `);
  const toFuncIndex = (funcName: any) => funcNames.indexOf(funcName);
  const threadIndex = 0;
