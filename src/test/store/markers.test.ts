...
import {selectedThreadSelectors, getMarkerSchemaByName} from 'firefox-profiler/selectors';
...
describe('selectors/getMarkerChartTimingAndBuckets', function () {
  ...
  describe('network markers', function () {
    function getTimingAndBucketsForNetworkMarkers(testMarkers: any) {
      const profile = getProfileWithMarkers(testMarkers);
      ...
      const networkCategory = ensureExists(profile.meta.categories).findIndex(
        (category: any) => category.name === 'Network'
      );
...
describe('memory markers', function () {
  ...
  it('can get memory markers using getMemoryMarkers', function () {
    ...
    expect(
      markerIndexes.map((markerIndex: any) => getMarker(markerIndex).name)
...
describe('selectors/getCommittedRangeAndTabFilteredMarkerIndexes', function () {
  ...
  function setup(ctxId: any, markers?: Array<any> | null) {
    const profile = getProfileWithMarkers(
      markers || [
        [
          'Dummy 1',
    ...
    const getMarker = selectedThreadSelectors.getMarkerGetter(getState());
    return markerIndexes.map((markerIndex: any) => getMarker(markerIndex).name);
  }
...
describe('Marker schema filtering', function () {
  ...
  function setup(profile: any) {
    ...
    function getMarkerNames(selector: any): string[] {
