...
describe('doSymbolicateProfile', function () {
  ...
  function init() {
    ...
    const symbolProvider = {
      ...
      requestSymbolTableFromBrowser: async (lib: any) => {
        if (lib.debugName !== 'firefox.pdb') {
  ...
  it('can re-symbolicate a partially-symbolicated profile even if it needs to add funcs to the funcTable', async () => {
    ...
    const newFuncTable = { ...funcTable, length: 2 } as const;
    const newNativeSymbols = { ...nativeSymbols, length: 2 } as const;
    const newThread = {
      ...thread,
      funcTable: newFuncTable,
      nativeSymbols: newNativeSymbols,
    } as const;
    const newProfile = { ...profile, threads: [newThread] } as const;
    dispatch({
      type: 'PROFILE_LOADED',
      profile: newProfile,
