...
describe('getRemoveProfileInformation', function () {
  ...
  describe('about non-active tab information', () => {
    function setupForActiveTab(updateChannel: any, timelineTrackOrganization: any) {
...
describe('attemptToPublish', function () {
  ...
  function setupFakeUpload() {
    ...
    const initUploadProcess: typeof uploadBinaryProfileData = () => ({
      ...
      startUpload: (data: any, callback: any) => {
  ...
  function setupFakeUploadsWithStore(store: Store) {
    ...
    function waitUntilPhase(phase: any) {
      return waitUntilState(store, (state: any) => getUploadPhase(state) === phase);
    }
