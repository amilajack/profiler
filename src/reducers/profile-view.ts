...
const viewOptionsPerThread: Reducer<ThreadViewOptionsPerThreads> = (state = ({} as ThreadViewOptionsPerThreads), action): ThreadViewOptionsPerThreads => {
  switch (action.type) {
    case 'PROFILE_LOADED':
    ...
    case 'CHANGE_SELECTED_CALL_NODE': {
      ...
      const expandToNodeParentPaths: Array<any | Array<IndexIntoFuncTable>> = [];
...
const wrapReducerInResetter = (regularReducer: Reducer<ProfileViewState>): Reducer<ProfileViewState> => {
  return (state: undefined | ProfileViewState, action) => {
    switch (action.type) {
