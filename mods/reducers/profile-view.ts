...
import * as Transforms from "~/components/FFP/profile-logic/transforms";
import * as ProfileData from "~/components/FFP/profile-logic/profile-data";
import { arePathsEqual, PathSet } from "~/components/FFP/utils/path";
...
} from "~/components/FFP/profile-logic/symbolication";
import type { TabSlug } from "~/components/FFP/app-logic/tabs-handling";

import { objectMap } from "~/components/FFP/utils/flow";
 ...
 * pass. It is valid for the lifetime of the profile.
 */
const activeTabTimeline: Reducer<ActiveTabTimeline | null> = (state = null, action) => {
  switch (action.type) {
...
function _getThreadViewOptions(state: ThreadViewOptionsPerThreads, threadsKey: ThreadsKey) {
...
const viewOptionsPerThread: Reducer<ThreadViewOptionsPerThreads> = (
  ...
  switch (action.type) {
    ...
    case "BULK_SYMBOLICATION": {
      ...
      const newState = objectMap(state, (threadViewOptions, threadsKey) => {
        ...
        if (Number.isNaN(threadIndex)) {
          throw new Error(
            "Bulk symbolication only supports a single thread, and a ThreadsKey with " + "multiple threads was used."
          );
        }
        ...
        return {
          ...
          expandedCallNodePaths: applyFuncSubstitutionToPathSetAndIncludeNewAncestors(
            oldFuncToNewFuncsMap,
    ...
    case "CHANGE_SELECTED_CALL_NODE": {
      const { selectedCallNodePath, threadsKey, optionalExpandedToCallNodePath } = action;
    ...
    case "CHANGE_INVERT_CALLSTACK": {
      ...
      return objectMap(state, (viewOptions, threadsKey) => {
        if (
          ...
          threadsKey === ProfileData.getThreadsKey(selectedThreadIndexes).toString()
        ) {
    ...
    case "CHANGE_IMPLEMENTATION_FILTER": {
      const { transformedThread, threadsKey, previousImplementation, implementation } = action;
...
const tableViewOptionsPerTab: Reducer<TableViewOptionsPerTab> = (
  ...
  switch (action.type) {
    case "CHANGE_TABLE_VIEW_OPTIONS":
      return _updateTableViewOptions(state, action.tab, action.tableViewOptions);
    default:
      return state;
  }
};
...
const lastNonShiftClick: Reducer<LastNonShiftClickInformation | null> = (state = null, action) => {
  switch (action.type) {
    case "SELECT_TRACK":
    ...
    case "HIDE_GLOBAL_TRACK": {
      ...
      if (clickedTrack.type === "global" && clickedTrack.trackIndex === action.trackIndex) {
...
const rightClickedCallNode: Reducer<RightClickedCallNode | null> = (state = null, action) => {
  switch (action.type) {
    case "BULK_SYMBOLICATION": {
      ...
      return {
        ...state,
        callNodePath: applyFuncSubstitutionToCallPath(oldFuncToNewFuncsMap, state.callNodePath),
      };
    }
...
const rightClickedMarker: Reducer<MarkerReference | null> = (state = null, action) => {
  switch (action.type) {
...
const hoveredMarker: Reducer<MarkerReference | null> = (state = null, action) => {
  switch (action.type) {
 ...
 */
const mouseTimePosition: Reducer<Milliseconds | null> = (state = null, action) => {
  switch (action.type) {
