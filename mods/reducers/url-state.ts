...
import { objectEntries } from "~/components/FFP/utils/flow";
import { BottomBoxTabSlug, tabSlugs } from "~/components/FFP/app-logic/tabs-handling";
...
const selectedBottomBoxTab: Reducer<BottomBoxTabSlug> = (state = "source-code", action) => {
  switch (action.type) {
    case "CHANGE_SELECTED_BOTTOM_BOX_TAB":
      return action.selectedBottomBoxTab;
    default:
      return state;
  }
};
...
const selectedThreads: Reducer<Set<ThreadIndex> | null> = (state = null, action) => {
  switch (action.type) {
...
const timelineType: Reducer<TimelineType> = (state = "cpu-category", action) => {
  switch (action.type) {
...
const hiddenGlobalTracks: Reducer<Set<TrackIndex>> = (state = new Set(), action) => {
  switch (action.type) {
    ...
    case "SHOW_PROVIDED_TRACKS": {
      ...
      action.globalTracksToShow.forEach(Set.prototype.delete, hiddenGlobalTracks);
      return hiddenGlobalTracks;
    }
...
const hiddenLocalTracksByPid: Reducer<Map<Pid, Set<TrackIndex>>> = (state = new Map(), action) => {
  switch (action.type) {
    ...
    case "SHOW_PROVIDED_TRACKS": {
      ...
      for (const [pid, localTracksToMakeVisible] of action.localTracksByPidToShow.entries()) {
        const hiddenLocalTracks = state.get(pid) ?? new Set();
        const newHiddenLocalTracks = new Set(hiddenLocalTracks);
        localTracksToMakeVisible.forEach(Set.prototype.delete, newHiddenLocalTracks);
    ...
    case "HIDE_PROVIDED_TRACKS": {
      ...
      for (const [pid, localTracksToMakeHidden] of action.localTracksByPidToHide.entries()) {
        const hiddenLocalTracks = state.get(pid) ?? new Set();
        const newHiddenLocalTracks = new Set(hiddenLocalTracks);
        localTracksToMakeHidden.forEach(Set.prototype.add, newHiddenLocalTracks);
        hiddenLocalTracksByPid.set(pid, newHiddenLocalTracks);
      }
...
const localTrackOrderChangedPids: Reducer<Set<Pid>> = (state = new Set(), action) => {
  switch (action.type) {
    case "CHANGE_LOCAL_TRACK_ORDER": {
...
const timelineTrackOrganization: Reducer<TimelineTrackOrganization> = (state = { type: "full" }, action) => {
  switch (action.type) {
    case "VIEW_FULL_PROFILE":
      return { type: "full" };
    case "VIEW_ACTIVE_TAB_PROFILE":
  ...
  switch (action.type) {
    case "UPDATE_BOTTOM_BOX": {
      return {
        ...
        allNativeSymbolsForInitiatingCallNode: action.allNativeSymbolsForInitiatingCallNode,
...
const isBottomBoxOpenPerPanel: Reducer<IsOpenPerPanelState> = (state = _getBottomBoxInitialState(), action) => {
  switch (action.type) {
    case "UPDATE_BOTTOM_BOX": {
...
const isCompareMode: Reducer<boolean> = (state = false, action) => {
  switch (action.type) {
    case "PROFILE_LOADED":
    case "VIEW_FULL_PROFILE":
      return !!action.isCompareMode;
    default:
      return state;
  }
};

const fetchRetryGeneration: Reducer<number> = (state = 0, action) => {
  switch (action.type) {
    case "RETRY_PROFILE_QUERY":
      return state + 1;
    default:
      return state;
  }
