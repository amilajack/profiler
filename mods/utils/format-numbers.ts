...
import type { Microseconds, Milliseconds, Nanoseconds, WeightType } from "~/components/FFP/types";
...
function _getNumberFormat({ places, style }: { places: number; style: "decimal" | "percent" }) {
 ...
 */
export function formatCallNodeNumber(weightType: WeightType, isHighPrecision: boolean, number: number): string {
 ...
 * Format call node numbers consistently.
 */
export function formatCallNodeNumberWithUnit(weightType: WeightType, isHighPrecision: boolean, number: number): string {
  switch (weightType) {
    case "tracing-ms": {
      // Sometimes the number should be high precision, such as on a JS tracer thread
      // which has timing to the microsecond.
...
const PERCENT_FORMAT_FRACTIONAL_DIGITS = 2;
const PERCENT_FORMAT_SIGNIFICANT_DIGITS = 3;
export const PERCENT_NEGLIGIBLE_DELTA = 0.01;
/**
...
export function formatPercent(ratio: number): string {
  const percent = formatNumber(
    ratio,
    /* significantDigits */ PERCENT_FORMAT_SIGNIFICANT_DIGITS,
    /* maxFractionalDigits */ PERCENT_FORMAT_FRACTIONAL_DIGITS,
    "percent"
  );

  if (ratio === 0) return "0.00%";
  if (ratio < PERCENT_NEGLIGIBLE_DELTA) return "~0.00%";
  return percent;
}

export function formatComparePercent(ratio: number): string {
  const percent = formatNumber(
    ratio,
    /* significantDigits */ PERCENT_FORMAT_SIGNIFICANT_DIGITS,
    /* maxFractionalDigits */ PERCENT_FORMAT_FRACTIONAL_DIGITS,
  ...
  if (ratio === 0) return "0.00%";
  if (Math.abs(ratio) < PERCENT_NEGLIGIBLE_DELTA) return "~0.00%";
  if (ratio > 0) return `+${percent}`;
  return percent;
}
...
export function formatBytes(bytes: number, significantDigits: number = 3, maxFractionalDigits: number = 2): string {
  if (bytes < 10000) {
    ...
    return formatNumber(bytes / 1024, significantDigits, maxFractionalDigits) + "KB";
  } else if (bytes < 1024 * 1024 * 1024) {
    return formatNumber(bytes / (1024 * 1024), significantDigits, maxFractionalDigits) + "MB";
  }
  return formatNumber(bytes / (1024 * 1024 * 1024), significantDigits, maxFractionalDigits) + "GB";
}
...
export function formatTimestamp(time: Milliseconds, significantDigits: number = 5, maxFractionalDigits: number = 3) {
...
export function formatValueTotal(
  a: number,
  b: number,
  formatNum: (arg1: number) => string = String,
