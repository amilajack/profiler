...
const ENCODING_DIGITS: string = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ._";
...
//                                                      ^^^^^^^ can be skipped
function countSkippableConsecutiveNumbersAt(numbers: number[], start: number): number {
  ...
  if (current === previous + 1 && next === current + 1) {
    ...
    skipCount = 1;
    while (start + skipCount + 1 < numbers.length && numbers[start + skipCount + 1] === current + skipCount + 1) {
      skipCount++;
    }
