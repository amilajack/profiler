...
export type ResizeObserverCallback = (arg1: DOMRectReadOnly) => unknown;
export type ResizeObserverWrapper = {
  subscribe: (elt: HTMLElement, arg2: ResizeObserverCallback) => void;
  unsubscribe: (elt: HTMLElement, arg2: ResizeObserverCallback) => void;
};
  ...
  return {
    ...
    unsubscribe(element: HTMLElement, callback: ResizeObserverCallback) {
      ...
      if (callbacksForElement) {
        ...
        if (!wasDeleted) {
          console.warn(`[ResizeObserverWrapper.unsubscribe] We tried to unregister an unknown callback.`);
        }
