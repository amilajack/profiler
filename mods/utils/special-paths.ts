...
const repoPathRegex = /^(?<vcs>hg|git):(?<repo>[^:]*):(?<path>[^:]*):(?<rev>.*)$/;
const s3PathRegex = /^s3:(?<bucket>[^:]*):(?<digest>[0-9a-f]*)\/(?<path>[^:]*):$/;
const cargoPathRegex = /^cargo:(?<registry>[^:]*):(?<crate>[^/]+)-(?<version>[0-9]+\.[0-9]+\.[0-9]+):(?<path>[^:]*)$/;
export function parseFileNameFromSymbolication(file: string): ParsedFileNameFromSymbolication {
  ...
  if (repoMatch !== null && repoMatch.groups) {
    ...
    if (vcs !== "hg" && vcs !== "git") {
      throw new Error('The regexp ensures that "vcs" is "hg" or "git", so this cannot happen.');
    }
