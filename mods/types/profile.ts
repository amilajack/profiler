...
import { Flow } from "~/components/FFP/types";
import type { Milliseconds, Address, Microseconds, Bytes } from "~/components/FFP/types/units";
import type { UniqueStringArray } from "~/components/FFP/utils/unique-string-array";
import type { MarkerPayload, MarkerSchema, MarkerFormatType } from "~/components/FFP/types/markers";
import type { MarkerPhase, ProfilingLog } from "~/components/FFP/types/gecko-profile";
...
export type SamplesLikeTable = SamplesLikeTableShape | SamplesTable | NativeAllocationsTable | JsAllocationsTable;
...
export type FuncTable = {
  ...
  // The _resource describes the internal name we use to fetch source maps.
  _resource: Array<IndexIntoResourceTable | -1>;
  ...
  // The thread from which this function was originally part of. Only used when merging threads for compare.
  threadOrigin?: Array<number | null>;
  // The function's uniqueness. Only returned by the backend when comparing profiles.
...
export type SampleUnits = {
  readonly time: "ms";
  readonly eventDelay: "ms";
  readonly threadCPUDelta: ThreadCPUDeltaUnit;
};
...
export type ProfileMeta = {
  // Palette Specific metadata fields
  sampleCount: number;
  projectId: string;
  // --------------------------------
  ...
  platform?: // It usually has the version embedded in the string
  | "Android"
    | "Windows" // X11 is used for historic reasons, but this value means that it is a Unix platform.
    | "Macintosh"
    | "X11"
    | string;
  ...
  updateChannel?: // Local builds
  | "default"
    | "nightly" // Nightly try builds for QA
    | "nightly-try" // Developer Edition channel
    | "aurora"
    | "beta"
    | "release" // Extended Support Release channel
    | "esr"
    | string;
...
type SerializableThread = Flow.Diff<
  Thread,
  {
    stringTable: UniqueStringArray;
  }
 ...
 */
export type SerializableProfile = Profile & {
