...
import type { Milliseconds, Microseconds, Seconds, Bytes } from "~/components/FFP/types/units";
import type { GeckoMarkerStack } from "~/components/FFP/types/gecko-profile";
import type { IndexIntoStackTable, IndexIntoStringTable, Tid, Pid } from "~/components/FFP/types/profile";
import type { MixedObject, ObjectMap } from "~/components/FFP/types/utils";
...
export type MarkerDisplayLocation =
  | "marker-chart" // This adds markers to the main marker timeline in the header.
...
export type $ReplaceCauseWithStack<T extends any> = T & {
...
export type ScreenshotPayload = // Markers that represent the closing of a window (name === 'CompositorScreenshotWindowDestroyed')
...
export type MarkerPayload_Gecko =
  ...
  | MediaSampleMarkerPayload
  | NoPayloadUserData // The following payloads come in with a stack property. During the profile processing
