...
import type { Milliseconds, StartEndRange, Address, Bytes } from "~/components/FFP/types/units";
import type { MarkerPayload } from "~/components/FFP/types/markers";
import type {
  IndexIntoFuncTable,
  ThreadIndex,
  Pid,
  ...
  TabID,
  Tid,
} from "~/components/FFP/types/profile";
import type { IndexedArray } from "~/components/FFP/types/utils";
import type { StackTiming } from "~/components/FFP/profile-logic/stack-timing";
...
/**
 * A map that converts a SelectedCallNodeIndex into a CompareCallNodeIndex.
 */
export type CallNodeIndexToCompareCallNodeIndexTable = Map<number, number | null | undefined>;
...
export type DerivedMarkerInfo = {
  markers: Marker[];
  markerIndexToRawMarkerIndexes: IndexedArray<MarkerIndex, IndexIntoRawMarkerTable[]>;
};
...
export type GlobalTrack = // mainThreadIndex is null when this is a fake global process added to contain
  // real threads.
  | {
...
export type LocalTrack =
  ...
  | {
      readonly type: "event-delay";
      readonly threadIndex: ThreadIndex;
    }
  | {
      readonly type: "process-cpu";
      readonly counterIndex: CounterIndex;
    }
  | {
      readonly type: "power";
...
export type OriginsTimelineTrack = OriginsTimelineEntry | OriginsTimelineRoot | OriginsTimelineNoOrigin;

export type OriginsTimeline = Array<OriginsTimelineNoOrigin | OriginsTimelineRoot>;
...
export type ActiveTabGlobalTrack = ActiveTabMainTrack | ActiveTabScreenshotTrack;
...
export type RemoveProfileInformation = {
  ...
  readonly shouldRemoveTabsExceptTabID: TabID | null;
};
...
export type SelectedState = // Samples can be filtered through various operations, like searching, or
  ...
  | "FILTERED_OUT_BY_TRANSFORM" // This sample is selected because either the tip or an ancestor call node matches
  // the currently selected call node.
  | "FILTERED_OUT_BY_ACTIVE_TAB" // This call node is not selected, and the stacks are ordered before the selected
  // call node as sorted by the getTreeOrderComparator.
  | "SELECTED" // This call node is not selected, and the stacks are ordered after the selected
  // call node as sorted by the getTreeOrderComparator.
...
export type TracedTiming = {
  readonly self: Float32Array;
  readonly running: Float32Array;
};
