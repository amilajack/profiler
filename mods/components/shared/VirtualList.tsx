...
import type { CssPixels } from "~/components/FFP/types";

type RenderItem<Item> = (arg1: Item, arg2: number, arg3: number) => React.ReactElement;
...
type VirtualListInnerChunkProps<Item> = {
  ...
  readonly renderItem: RenderItem<Item>;
  readonly items: ReadonlyArray<Item>;
  readonly specialItems: ReadonlyArray<Item | undefined>;
...
class VirtualListInnerChunk<Item> extends React.PureComponent<VirtualListInnerChunkProps<Item>> {
  render() {
    const { className, renderItem, items, specialItems, visibleRangeStart, visibleRangeEnd, columnIndex, forceRender } =
      this.props;
  ...
  render() {
    ...
    return (
      <div
        ...
        style={{
          height: `${items.length * itemHeight + 12}px`,
...
type VirtualListProps<Item> = {
  ...
  readonly onCopy?: (arg1: ClipboardEvent) => void;
...
export class VirtualList<Item> extends React.PureComponent<VirtualListProps<Item>> {
  _container: {
    current: HTMLDivElement | null;
  } = React.createRef();
  _inner: {
    current: VirtualListInner<Item> | null;
  } = React.createRef();
  ...
  _queryGeometry(): Geometry | undefined {
    const container = this._container.current;
   ...
   * part.
   */
  _scrollContainerHorizontally(container: HTMLDivElement, itemX: CssPixels, offsetX: CssPixels) {
    ...
    if (container.scrollLeft > itemLeft) {
      ...
      container.scrollLeft = Math.min(itemLeft, itemRight - scrollingColumnWidth);
    }
  }
  ...
  /* This method is used by users of this component. */
  /* eslint-disable-next-line react/no-unused-class-component-methods */
  scrollItemIntoView(itemIndex: number, itemX: CssPixels, offsetX: CssPixels = 0) {
    const container = this._container.current;
    if (!container) {
      return;
    }
    ...
    if (
      ...
      const scrollTopToCenterItem = itemTop - (container.clientHeight - this.props.itemHeight) / 2;
    ...
    } else if (itemBottomWithMargin > container.scrollTop + container.clientHeight) {
      ...
      container.scrollTop = Math.min(itemTopWithMargin, itemBottomWithMargin - container.clientHeight);
    }
  ...
  render() {
    ...
    return (
      ...
      >
        <div className={`${className}InnerWrapper`} onMouseLeave={this._onMouseLeaveInnerWrapper}>
          {range(columnCount).map((columnIndex) => (
            <VirtualListInner
              className={classNames(`${className}Inner`, `${className}Inner${columnIndex}`)}
