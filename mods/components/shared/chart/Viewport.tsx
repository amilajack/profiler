...
import { setHasZoomedViaMousewheel } from "~/components/FFP/actions/app";
import { updatePreviewSelection } from "~/components/FFP/actions/profile-view";
import { getHasZoomedViaMousewheel, getPanelLayoutGeneration } from "~/components/FFP/selectors/app";
import explicitConnect from "~/components/FFP/utils/connect";
import { getResizeObserverWrapper } from "~/components/FFP/utils/resize-observer-wrapper";

import type { CssPixels, PreviewSelection, StartEndRange, UnitIntervalOfProfileRange } from "~/components/FFP/types";

import type { ConnectedProps } from "~/components/FFP/utils/connect";
import { assertExhaustiveCheck, getObjectValuesAsUnion } from "~/components/FFP/utils/flow";
...
export type Viewport = {
  ...
  readonly moveViewport: (arg1: CssPixels, arg2: CssPixels) => void;
...
type ViewportOwnProps<ChartProps> = {
  readonly viewportProps: {
    ...
    readonly containerRef?: (arg1: HTMLDivElement | null) => void;
...
export type WithChartViewport<
  ...
  ChartOwnProps extends any, // The chart component's props are given the viewport object, as well as the original
  // ChartOwnProps.
  ChartProps extends Readonly<
    ChartOwnProps & {
      viewport: Viewport;
    }
  >
...
// generic parameters.
export const withChartViewport: WithChartViewport<any, any> =
  ...
  <ChartOwnProps extends Record<string, unknown>>(ChartComponent: React.ComponentType<any>): any => {
    ...
    class ChartViewport extends React.PureComponent<ViewportProps, State> {
      ...
      _pendingPreviewSelectionUpdates: Array<(arg1: HorizontalViewport) => PreviewSelection> = [];
      ...
      UNSAFE_componentWillReceiveProps(newProps: ViewportProps) {
        if (this.props.viewportProps.viewportNeedsUpdate(this.props.chartProps, newProps.chartProps)) {
          ...
          this.props.viewportProps.previewSelection !== newProps.viewportProps.previewSelection ||
       ...
       */
      _addBatchedPreviewSelectionUpdate(callback: (arg1: HorizontalViewport) => PreviewSelection) {
      ...
      zoomRangeSelection = (center: number, zoomFactor: number) => {
        const { disableHorizontalMovement, maximumZoom } = this.props.viewportProps;
        if (disableHorizontalMovement) {
          return;
        }

        this._addBatchedPreviewSelectionUpdate(({ viewportLeft, viewportRight }) => {
      ...
      _keyboardNavigation = (timestamp: number) => {
        ...
        const delta = Math.min(
          KEYBOARD_NAVIGATION_SPEED * (timestamp - this._lastKeyboardNavigationFrame) * 0.001,
      ...
      moveViewport = (offsetX: CssPixels, offsetY: CssPixels): void => {
        const {
          viewportProps: { maxViewportHeight, timeRange, startsAtBottom, disableHorizontalMovement },
        } = this.props;
        const { containerWidth, containerHeight, viewportTop, isSizeSet } = this.state;
      ...
      render() {
        ...
        return (
          <div
            className={clsx("relative flex-1 overflow-hidden outline-0", isDragging && "cursor-grabbing", className)}
 ...
 */
function getNormalizedScrollDelta(event: WheelEvent, pageHeight: number, key: "deltaY" | "deltaX"): CssPixels {
