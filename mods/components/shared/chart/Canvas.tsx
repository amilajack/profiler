...
import { Tooltip } from "~/components/FFP/components/shared/Tooltip";

import type { CssPixels, DevicePixels } from "~/components/FFP/types";

type Props<HoveredItem> = {
  readonly containerWidth: CssPixels;
  readonly containerHeight: CssPixels;
  readonly className: string;
  readonly onSelectItem?: (arg1: HoveredItem | null) => void;
  readonly onRightClick?: (arg1: HoveredItem | null) => void;
  readonly onDoubleClickItem: (arg1: HoveredItem | null) => void;
  readonly getHoveredItemInfo: (arg1: HoveredItem) => React.ReactNode;
  readonly drawCanvas: (
...
export class ChartCanvas<HoveredItem> extends React.Component<Props<HoveredItem>, State<HoveredItem>> {
  ...
  _scheduleDraw(isHoveredOnlyDifferent: boolean = false, prevHoveredItem: HoveredItem | null = null) {
    ...
    if (drawCanvasAfterRaf === false) {
      // The behavior of waiting for a rAF has been explicitly  disabled.
  ...
  _doDrawCanvas(isHoveredOnlyDifferent: boolean = false, prevHoveredItem: HoveredItem | null = null) {
    const { drawCanvas } = this.props;
    if (this._canvas) {
      timeCode(`render canvas`, () => {
        this._prepCanvas();

        drawCanvas(
          this._ctx,
          // don't show hovered item while dragging
          this.props.isDragging ? null : this.state.hoveredItem,
          prevHoveredItem,
  ...
  _onMouseMove = (
    ...
    if (
      ...
      hasLeftClick &&
      (Math.abs(this._offsetX - this._mouseDownOffsetX) > MOUSE_CLICK_MAX_MOVEMENT_DELTA ||
        Math.abs(this._offsetY - this._mouseDownOffsetY) > MOUSE_CLICK_MAX_MOVEMENT_DELTA)
    ) {
  ...
  _getHoveredItemInfo = (): React.ReactNode | null => {
  ...
  render() {
    ...
    return (
      <div>
        <canvas
          className={clsx("absolute inset-0", this.props.className, hoveredItem !== null && !isDragging && "hover")}
