...
export class Tooltip extends React.PureComponent<Props> {
  ...
  // "after" / "after" is the preferred positioning, so it's our default.
  ...
  _computeNewPosition({
    ...
    switch (possiblePositions.length) {
      ...
      case 2:
        ...
        newPosition = previousPosition !== "window-edge" ? previousPosition : "before-mouse";
        break;
      default:
        throw new Error(`We got more than 2 possible positions ${possiblePositions.length}, which shouldn't happen.`);
    }
  ...
  setPositioningStyle() {
    ...
    // normalize the vertical position to account for the scroll position
    const scrollY = window.scrollY;

    interiorElement.style.left = horizontalResult.style + "px";
    interiorElement.style.top = verticalResult.style - scrollY + "px";
  ...
  render() {
    return (
      <Portal.Root>
        <div
          className="pointer-events-none fixed z-[100] flex min-w-[160px] max-w-2xl flex-col overflow-hidden rounded border bg-white bg-clip-padding py-1 text-xs shadow-lg"
