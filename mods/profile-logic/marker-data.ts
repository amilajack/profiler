...
import { getEmptyRawMarkerTable } from "~/components/FFP/profile-logic/data-structures";
import { getFriendlyThreadName } from "~/components/FFP/profile-logic/profile-data";
import { removeFilePath, removeURLs } from "~/components/FFP/utils/string";
import { ensureExists, assertExhaustiveCheck } from "~/components/FFP/utils/flow";
import { INSTANT, INTERVAL, INTERVAL_START, INTERVAL_END } from "~/components/FFP/app-logic/constants";
import {
  getMarkerSchemaName,
  getSchemaFromMarker,
  markerPayloadMatchesSearch,
} from "~/components/FFP/profile-logic/marker-schema";
...
export class IPCMarkerCorrelations {
  ...
  get(tid: Tid, index: number): IPCSharedData | null | undefined {
...
export function correlateIPCMarkers(threads: Thread[]): IPCMarkerCorrelations {
  ...
  function formatThreadName(tid?: number | null): string | undefined {
    ...
    if (data.direction === "sending") {
      ...
      switch (phase) {
        ...
        case undefined:
          return 4;
        case "transferStart":
          throw new Error('Unexpected "transferStart" phase found on receiving side.');
        default:
          throw assertExhaustiveCheck(phase, `Unhandled IPC phase type.`);
      }
    }
  ...
  for (const markers of markersByKey.values()) {
    ...
    const sharedData: IPCSharedData = {
      startTime: startEndpointMarker ? startEndpointMarker.data.startTime : undefined,
    ...
    if (startEndpointMarker) {
      ...
      correlations.set(startEndpointMarker.tid, startEndpointMarker.index, sharedData);
    }
    if (endEndpointMarker) {
      addedThreadIds.add(endEndpointMarker.tid);
      correlations.set(endEndpointMarker.tid, endEndpointMarker.index, sharedData);
    }
...
export function deriveMarkersFromRawMarkerTable(
  ...
  const markerIndexToRawMarkerIndexes: IndexedArray<MarkerIndex, IndexIntoRawMarkerTable[]> = [];
  ...
  const openIntervalMarkers: Map<IndexIntoStringTable, MarkerIndex[]> = new Map();
  ...
  function mergeIntervalData(startData: MarkerPayload | null, endData: MarkerPayload | null): MarkerPayload | null {
  ...
  for (let rawMarkerIndex = 0; rawMarkerIndex < rawMarkers.length; rawMarkerIndex++) {
    ...
    const markerThreadId = rawMarkers.threadId ? rawMarkers.threadId[rawMarkerIndex] : null;
    ...
    if (data) {
      switch (data.type) {
        case "Network": {
          ...
          const ensureMessage = "Network markers are assumed to have a start and end time.";
          if (data.status === "STATUS_START") {
            ...
            if (startIndex !== undefined) {
              ...
              const startStartTime = ensureExists(rawMarkers.startTime[startIndex], ensureMessage);
              ...
              const start = Math.min(
                threadRange.start,
                ensureExists(maybeStartTime, "Network markers are assumed to have a start time.")
              );
        ...
        case "CompositorScreenshot": {
          ...
          const { windowID } = data;
          const previousScreenshotMarker = previousScreenshotMarkers.get(windowID);
          if (previousScreenshotMarker !== undefined) {
            ...
            const markerThreadId = rawMarkers.threadId ? rawMarkers.threadId[previousScreenshotMarker] : null;
          ...
          if (stringTable.getString(name) === "CompositorScreenshotWindowDestroyed") {
        ...
        case "IPC": {
          ...
          if (data.direction === "sending" && data.phase === "transferEnd" && sharedData.sendStartTime !== undefined) {
          ...
          let start = ensureExists(data.startTime, "Expected IPC startTime to exist in the payload.");
          let end = start;
          let incomplete = true;
          if (sharedData.startTime !== undefined && sharedData.endTime !== undefined) {
    ...
    switch (phase) {
      case INSTANT:
        addMarker([rawMarkerIndex], {
          start: ensureExists(maybeStartTime, "An Instant marker did not have a startTime."),
          end: null,
          name: stringTable.getString(name),
          category,
          threadId: markerThreadId,
      ...
      case INTERVAL:
        {
          const startTime = ensureExists(maybeStartTime, "An Interval marker did not have a startTime.");
          const endTime = ensureExists(maybeEndTime, "An Interval marker did not have a startTime.");
      ...
      case INTERVAL_END:
        {
          ...
          const endTime = ensureExists(maybeEndTime, "An IntervalEnd marker did not have an endTime");
  ...
  for (const markerBucket of openIntervalMarkers.values()) {
    for (const startIndex of markerBucket) {
      const start = ensureExists(
        rawMarkers.startTime[startIndex],
        "Encountered a marker without a startTime. Eventually this needs to be handled " + "for phase-style markers."
      );
  ...
  for (const previousScreenshotMarker of previousScreenshotMarkers.values()) {
    ...
    addMarker([previousScreenshotMarker], {
      ...
      threadId: rawMarkers.threadId ? rawMarkers.threadId[previousScreenshotMarker] : null,
...
export function filterRawMarkerTableToRangeWithMarkersToDelete(
  ...
  if (filterRange === null) {
    ...
    const filteredMarkerIndexes = filterRawMarkerTableIndexesToRange(oldMarkerTable, derivedMarkerInfo, start, end);
...
export function filterMarkerIndexes(
  getMarker: (arg1: MarkerIndex) => Marker,
  markerIndexes: MarkerIndex[],
  filterFunc: (arg1: Marker) => boolean
): MarkerIndex[] {
...
export function filterMarkerIndexesToRange(
  ...
  return filterMarkerIndexes(
    getMarker,
    markerIndexes,
    (marker) => marker.start <= rangeEnd && (marker.end || marker.start) >= rangeStart
  );
}
 ...
 */
export function isOnThreadFileIoMarker(marker: Marker): boolean | undefined {
  const { data } = marker;
...
): "network-color-css" | "network-color-js" | "network-color-html" | "network-color-img" | "network-color-other" {
  switch (mimeType) {
    case "text/css":
 ...
 */
export function sanitizeTextMarker(payload: TextMarkerPayload): TextMarkerPayload {
  return {
    ...payload,
 ...
 */
export function sanitizeExtensionTextMarker(markerName: string, payload: TextMarkerPayload): TextMarkerPayload {
...
export function sanitizeFromMarkerSchema(markerSchema: MarkerSchema, markerPayload: MarkerPayload): MarkerPayload {
  for (const propertyDescription of markerSchema.data) {
    if (propertyDescription.key !== undefined && propertyDescription.format !== undefined) {
 ...
 */
export function getMarkerTypesForDisplay(markerSchema: MarkerSchema[], displayArea: string): Set<string> {
  const types = new Set();
...
export function filterMarkerByDisplayLocation(
  ...
  // or undefined if not.
  preemptiveFilterFunc: (data: Marker) => boolean | undefined = _doNotAutomaticallyAdd
): MarkerIndex[] {
  ...
  return filterMarkerIndexes(getMarker, markerIndexes, (marker) => {
    ...
    return markerTypes.has(getMarkerSchemaName(markerSchemaByName, marker.name, marker.data));
  });
}
...
export function computeScreenshotSize(
  ...
  const coefficient = windowHeight > windowWidth ? maximumSize / windowHeight : maximumSize / windowWidth;
