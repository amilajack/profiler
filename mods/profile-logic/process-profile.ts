...
import { AddressLocator } from "~/components/FFP/profile-logic/address-locator";
import {
  getEmptyExtensions,
  getEmptyFuncTable,
...
import { attemptToUpgradeProcessedProfileThroughMutation } from "~/components/FFP/profile-logic/processed-profile-versioning";
import { getOrCreateURIResource } from "~/components/FFP/profile-logic/profile-data";
import { ensureExists, immutableUpdate } from "~/components/FFP/utils/flow";
import { UniqueStringArray } from "~/components/FFP/utils/unique-string-array";
import { INSTANT, INTERVAL, INTERVAL_END } from "../app-logic/constants";
...
function _toStructOfArrays(geckoTable: any): any {
  const result = { length: geckoTable.data.length } as const;
...
export function extractFuncsAndResourcesFromFrameLocations(
  ...
  stringTable: UniqueStringArray,
  libs: LibMapping[],
  extensions: ExtensionTable | null | undefined = getEmptyExtensions(),
  globalDataCollector: GlobalDataCollector
): {
  funcTable: FuncTable;
  resourceTable: ResourceTable;
  ...
  for (let frameIndex = 0; frameIndex < frameLocations.length; frameIndex++) {
    ...
    const info = extractionInfo.stringToNewFuncIndexAndFrameAddress.get(locationString);
    if (info !== undefined) {
    ...
    const unsymbolicatedInfo = _extractUnsymbolicatedFunction(extractionInfo, locationString, locationIndex);
...
function _extractUnsymbolicatedFunction(
  ...
  const { addressLocator, libToResourceIndex, resourceTable, funcTable, stringTable } = extractionInfo;
...
function _extractCppFunction(extractionInfo: ExtractionInfo, locationString: string): IndexIntoFuncTable | null {
  ...
  const { funcTable, stringTable, stringToNewFuncIndexAndFrameAddress, libNameToResourceIndex, resourceTable } =
    extractionInfo;
...
function _extractJsFunction(extractionInfo: ExtractionInfo, locationString: string): IndexIntoFuncTable | null {
  ...
  const { funcTable, stringTable, resourceTable, originToResourceIndex } = extractionInfo;
 ...
 */
function _convertPayloadStackToIndex(data: MarkerPayload_Gecko | null): IndexIntoStackTable | null {
  if (!data) {
    return null;
  }
      ...
      switch (geckoPayload.type) {
        case "JS allocation": {
          ...
          jsAllocations.time.push(
            ensureExists(geckoMarkers.startTime[markerIndex], "JS Allocations are assumed to have a startTime")
          );
        ...
        case "Native allocation": {
          ...
          inProgressNativeAllocations.time.push(
            ensureExists(geckoMarkers.startTime[markerIndex], "Native Allocations are assumed to have a startTime")
          );
          ...
          if (hasMemoryAddresses) {
            ...
            threadId.push(
              ensureExists(geckoPayload.threadId, "Could not find a threadId property on a gecko marker payload.")
            );
          }
          // Do not process the marker and add it to the marker list.
          continue;
...
function convertPhaseTimes(old_phases: PhaseTimes<Milliseconds>): PhaseTimes<Microseconds> {
 ...
 * the GC information.
 */
function _processMarkerPayload(geckoPayload: MarkerPayload_Gecko | null): MarkerPayload | null {
  if (!geckoPayload) {
    return null;
  }
  ...
  switch (payload.type) {
    ...
    case "IPC": {
      ...
      const { startTime, endTime, otherPid, messageType, messageSeqno, side, direction, phase, sync, threadId } =
        payload;
      return {
        type: "IPC",
        startTime,
...
function _processSamples(geckoSamples: GeckoSampleStruct): SamplesTable {
  ...
  if (geckoSamples.threadCPUDelta) {
    ...
    const hasCPUDeltaValues = geckoSamples.threadCPUDelta.some((val) => val !== null);
...
function _processCounters(
  ...
  return geckoCounters.reduce<Array<any>>((result, { name, category, description, sample_groups }) => {
...
function processMarkerSchema(geckoProfile: GeckoProfile): MarkerSchema[] {
  ...
  const names: Set<string> = new Set(geckoProfile.meta.markerSchema.map(({ name }) => name));
 ...
 */
export function makeProfileSerializable({ threads, ...restOfProfile }: Profile): SerializableProfile {
  return {
    ...restOfProfile,
 ...
 */
function _unserializeProfile({ threads, ...restOfProfile }: SerializableProfile): Profile {
  return {
    ...restOfProfile,
 ...
 */
export async function unserializeProfileOfArbitraryFormat(arbitraryFormat: unknown): Promise<Profile> {
  try {
    ...
    if (String(arbitraryFormat) === "[object ArrayBuffer]") {
      ...
      // Flow here.
      const arrayBuffer: ArrayBuffer = arbitraryFormat as any;
      ...
      try {
        ...
        throw new Error("The profile array buffer could not be parsed as a UTF-8 string.");
      }
      // }
    }
    ...
    const processedProfile = attemptToUpgradeProcessedProfileThroughMutation(json);
    if (processedProfile) {
    ...
    throw new Error("The profile could not be parsed.");
  } catch (e: any) {
 ...
 */
export function processVisualMetrics(threads: Thread[], meta: ProfileMeta, pages: PageList) {
  ...
  const mainThreadIdx = threads.findIndex((thread) => thread.name === "GeckoMain" && thread.processType === "default");
  ...
  const testingCategoryIdx = meta.categories.findIndex((cat) => cat.name === "Test");
  ...
  if (tabThread.stringTable.hasString("Navigation::Start")) {
    const navigationStartStrIdx = tabThread.stringTable.indexForString("Navigation::Start");
    const navigationStartMarkerIdx = tabThread.markers.name.findIndex((m) => m === navigationStartStrIdx);
 ...
 */
function findTabMainThreadForVisualMetrics(threads: Thread[], pages: PageList): ThreadIndex | null {
  for (let threadIdx = 0; threadIdx < threads.length; threadIdx++) {
    ...
    const topLevelPagesSet = new Set(
      pages.filter((page) => page.embedderInnerWindowID === 0).map((page) => page.innerWindowID)
    );
    ...
    const refreshDriverTickStrIndex = thread.stringTable.indexForString("RefreshDriverTick");
