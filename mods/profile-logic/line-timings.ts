...
export function getStackLineInfo(
  ...
  return isInvertedTree
    ? getStackLineInfoInverted(stackTable, frameTable, funcTable, fileNameStringIndex)
    : getStackLineInfoNonInverted(stackTable, frameTable, funcTable, fileNameStringIndex);
}
...
export function getStackLineInfoNonInverted(
  ...
  const selfLineForAllStacks: Array<LineNumber | null> = [];
  ...
  for (let stackIndex = 0; stackIndex < stackTable.length; stackIndex++) {
    ...
    let totalLines: Set<LineNumber> | null = prefixStack !== null ? totalLinesForAllStacks[prefixStack] : null;
...
export function getStackLineInfoForCallNode(
  ...
  return isInvertedTree
    ? getStackLineInfoForCallNodeInverted(stackTable, frameTable, callNodeIndex, callNodeInfo)
    : getStackLineInfoForCallNodeNonInverted(stackTable, frameTable, callNodeIndex, callNodeInfo);
}
...
export function getStackLineInfoForCallNodeNonInverted(
  ...
  for (let stackIndex = 0; stackIndex < stackTable.length; stackIndex++) {
    ...
    if (stackIndexToCallNodeIndex[stackIndex] === callNodeIndex) {
      ...
      // totalTime.
      const prefixStack = stackTable.prefix[stackIndex];
      totalLines = prefixStack !== null ? callNodeTotalLinesForAllStacks[prefixStack] : null;
    }
...
export function getStackLineInfoForCallNodeInverted(
  ...
  // "self line" == "the line which a stack's self time is contributed to"
  const callNodeSelfLineForAllStacks: Array<LineNumber | null> = [];
  // "total lines" == "the set of lines whose total time this stack contributes to"
  // Either null or a single-element set.
  const callNodeTotalLinesForAllStacks: Array<Set<LineNumber> | null> = [];
...
export function getLineTimings(stackLineInfo: StackLineInfo | null, samples: SamplesLikeTable): LineTimings {
