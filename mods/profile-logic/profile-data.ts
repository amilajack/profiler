...
} from "~/components/FFP/profile-logic/data-structures";
import { INSTANT, INTERVAL, INTERVAL_START, INTERVAL_END } from "~/components/FFP/app-logic/constants";
import { timeCode } from "~/components/FFP/utils/time-code";
import { hashPath } from "~/components/FFP/utils/path";
import { bisectionRight, bisectionLeft } from "~/components/FFP/utils/bisect";
import { parseFileNameFromSymbolication } from "~/components/FFP/utils/special-paths";
...
export function getCallNodeInfo(
  ...
  return timeCode("getCallNodeInfo", () => {
    ...
    const sourceFramesInlinedIntoSymbol: Array<IndexIntoNativeSymbolTable | -1 | null> = [];
    const stackId: Array<Array<number>> = [];
    let length = 0;
    ...
    for (let stackIndex = 0; stackIndex < stackTable.length; stackIndex++) {
      ...
      const inlinedIntoSymbol = frameTable.inlineDepth[frameIndex] > 0 ? frameTable.nativeSymbol[frameIndex] : null;
      const funcIndex = frameTable.func[frameIndex];
      const prefixCallNodeAndFuncIndex = prefixCallNode * funcCount + funcIndex;
      const stackIdIndex = stackTable.stackId[stackIndex];
      ...
      if (callNodeIndex === undefined) {
        ...
        // Save the stackIds when merging call nodes.
        stackId[callNodeIndex].push(stackIdIndex);
        ...
        // into one call node.
        if (sourceFramesInlinedIntoSymbol[callNodeIndex] !== inlinedIntoSymbol) {
    ...
    const callNodeTable: CallNodeTable = {
      ...
      stackId: stackId.map((ids) => new Int32Array(ids)),
...
export function getSampleIndexToCallNodeIndex(
  ...
  stackIndexToCallNodeIndex: Partial<Record<IndexIntoStackTable, IndexIntoCallNodeTable>>
): Array<IndexIntoCallNodeTable | null> {
  return stacks.map((stack) => {
    return stack === null ? null : stackIndexToCallNodeIndex[stack];
  }) as Array<IndexIntoCallNodeTable | null>;
}
...
export function getSamplesSelectedStates(
  ...
  const selectedCallNodeDepth =
    selectedCallNodeIndex === -1 || selectedCallNodeIndex === null ? 0 : callNodeTable.depth[selectedCallNodeIndex];
...
export type JsImplementation = "interpreter" | "blinterp" | "baseline" | "ion" | "unknown";
export type StackImplementation = "native" | JsImplementation;
export type BreakdownByImplementation = Partial<Record<StackImplementation, Milliseconds>>;
...
export function getTimingsForCallNodeIndex(
  ...
  const javascriptCategoryIndex = categories.findIndex(({ name }) => name === "JavaScript");
   ...
   */
  function getImplementationForNativeStack(unfilteredStackIndex: IndexIntoStackTable): StackImplementation {
    ...
    for (
      let currentStackIndex: number | null = unfilteredStackIndex;
   ...
   */
  function getImplementationForJsStack(unfilteredFrameIndex: IndexIntoFrameTable): JsImplementation {
    const jsImplementationStrIndex = unfilteredFrameTable.implementation[unfilteredFrameIndex];
  ...
  function getImplementationForStack(thisSampleIndex: IndexIntoSamplesTable): StackImplementation {
    const stackIndex = unfilteredSamples.stack[thisSampleIndex + sampleIndexOffset];
    if (stackIndex === null) {
...
function _getTimeRangeForThread(
  ...
  const result = { start: Infinity, end: -Infinity } as const;
    ...
    for (let i = 0; i < markers.length; i++) {
      ...
      switch (markerPhase) {
        ...
        case INTERVAL: {
          ...
          result.end = Math.max(result.end, startTime + interval, endTime + interval);
          break;
        }
        default:
...
export function filterThreadByImplementation(
  ...
  const { funcTable, stringTable } = thread; // js0
...
function _filterThreadByFunc(
  thread: Thread,
  filter: (arg1: IndexIntoFuncTable) => boolean,
  ...
  return timeCode("filterThread", () => {
    ...
    const oldStackToNewStack = new Map<number, number>();
    const frameCount = frameTable.length;
    const prefixStackAndFrameToStack = new Map(); // prefixNewStack * frameCount + frame => newStackIndex

    function convertStack(stackIndex: number | IndexIntoStackTable): IndexIntoStackTable | null {
      ...
      if (newStack === undefined) {
        ...
        if (filter(funcIndex)) {
          ...
          if (newStack === undefined) {
            ...
            newStackTable.stackId[newStack] = stackTable.stackId[stackIndex];
...
export function filterThreadToSearchString(thread: Thread, searchString: string): Thread {
  ...
  const { funcTable, frameTable, stackTable, stringTable, resourceTable } = thread;

  function computeFuncMatchesFilter(func: number) {
  ...
  function funcMatchesFilter(func: number) {
  ...
  function stackMatchesFilter(stackIndex: number | null) {
    if (stackIndex === null) {
      return false;
    }
...
export function getInclusiveSampleIndexRangeForSelection(
  ...
  let [sampleStart, sampleEnd] = getSampleIndexRangeForSelection(table, rangeStart, rangeEnd);
...
export function filterThreadSamplesToRange(thread: Thread, rangeStart: number, rangeEnd: number): Thread {
  ...
  const newSamples: SamplesTable = {
    ...
    weight: samples.weight ? samples.weight.slice(beginSampleIndex, endSampleIndex) : null,
  ...
  if (samples.threadCPUDelta) {
    newSamples.threadCPUDelta = samples.threadCPUDelta.slice(beginSampleIndex, endSampleIndex);
  }

  if (samples.threadId) {
    newSamples.threadId = samples.threadId.slice(beginSampleIndex, endSampleIndex);
  }
  ...
  if (nativeAllocations) {
    ...
    const weight = nativeAllocations.weight.slice(startAllocIndex, endAllocIndex);
 ...
 */
export function filterCounterSamplesToRange(counter: Counter, rangeStart: number, rangeEnd: number): Counter {
  const newCounter = { ...counter } as const;
  newCounter.sampleGroups = [...newCounter.sampleGroups];
  const { sampleGroups } = newCounter;

  for (let sampleGroupIdx = 0; sampleGroupIdx < sampleGroups.length; sampleGroupIdx++) {
    ...
    // look cut off.
    const [beginSampleIndex, endSampleIndex] = getInclusiveSampleIndexRangeForSelection(
      sampleGroup.samples,
      rangeStart,
      rangeEnd
    );
...
export function processCounter(counter: Counter): Counter {
  const processedGroups = counter.sampleGroups.map((sampleGroup) => {
    ...
    const number = samples.number !== undefined ? samples.number.slice() : undefined;
...
export function accumulateCounterSamples(
  ...
  const accumulatedSamples = samplesArray.map((samples, index) => {
    ...
    const startSampleIndex = sampleRanges && sampleRanges[index] ? sampleRanges[index][0] : 0;
    const endSampleIndex = sampleRanges && sampleRanges[index] ? sampleRanges[index][1] : samples.length;
...
export function computeMaxCounterSampleCountsPerMs(
  ...
  const maxSampleCounts = samplesArray.map((samples, index) => {
    ...
    for (let i = startSampleIndex; i < endSampleIndex; i++) {
      ...
      const sampleTimeDeltaInMs = i === 0 ? profileInterval : samples.time[i] - samples.time[i - 1];
 ...
 */
export function processEventDelays(samples: SamplesTable, interval: Milliseconds): EventDelayInfo {
  if (!samples.eventDelay) {
  ...
  const rawEventDelays = ensureExists(samples.eventDelay, "eventDelays field is not present in this profile");
...
function _getCallNodeIndexFromPathWithCache(
  ...
  while (i < callNodePath.length) {
    ...
    const nextNodeIndex = getCallNodeIndexFromParentAndFunc(index, func, callNodeTable);
...
export function convertStackToCallNodeAndCategoryPath(
  ...
  const { stackTable, frameTable } = thread;
  const path = [];
  for (let stackIndex: number | null = stack; stackIndex !== null; stackIndex = stackTable.prefix[stackIndex]) {
 ...
 * 0 is returned.
 */
export function computeCallNodeMaxDepth(samples: SamplesLikeTable, callNodeInfo: CallNodeInfo): number {
...
export function invertCallstack(thread: Thread, defaultCategory: IndexIntoCategoryList): Thread {
  return timeCode("invertCallstack", () => {
    ...
    function stackFor(prefix: number, frame: number, category: number, subcategory: number, stackId: number) {
      ...
      if (stackIndex === undefined) {
        ...
        newStackTable.stackId[stackIndex] = stackId;
    ...
    function convertStack(stackIndex: number | null) {
      if (stackIndex === null) {
        return null;
      }
      let newStack = oldStackToNewStack.get(stackIndex);
      if (newStack === undefined) {
        newStack = null;
        for (
          let currentStack: number | null = stackIndex;
          currentStack !== null;
          ...
          newStack = stackFor(
            ...
            stackTable.stackId[currentStack]
          );
        }
        oldStackToNewStack.set(stackIndex, newStack);
      }
...
export function updateThreadStacks(
  ...
  convertStack: (arg1: IndexIntoStackTable | null) => IndexIntoStackTable | null
): Thread {
...
export function getMapStackUpdater(
  oldStackToNewStack: Map<null | IndexIntoStackTable, null | IndexIntoStackTable>
): (arg1: IndexIntoStackTable | null) => IndexIntoStackTable | null {
...
export function getSampleIndexClosestToStartTime(
  ...
  const distanceToLast = time - (samples.time[previousIndex] + previousWeight / 2);
 ...
 * the sample.
 */
export function getSampleIndexClosestToCenteredTime(samples: SamplesTable, time: number): IndexIntoSamplesTable {
  // Bisect to find the index of the first sample after the provided time.
  const index = bisectionRight(samples.time, time);
...
export function getThreadProcessDetails(thread: Thread, friendlyThreadName: string): string {
...
export function getOriginAnnotationForFunc(
  ...
  if (fileName) {
    if (origin && _shouldShowBothOriginAndFileName(fileName, origin, resourceType)) {
...
/**
 * Simplified version of `getOriginAnnotationForFunc` that only returns the
 * filename and line number.
 */
export function getFileNameAnnotationForFunc(
  funcIndex: IndexIntoFuncTable,
  funcTable: FuncTable,
  stringTable: UniqueStringArray
): string {
  const fileNameIndex = funcTable.fileName[funcIndex];
  if (fileNameIndex === null) return "";
...
export function getMetadataAnnotationForFunc(
  funcIndex: IndexIntoFuncTable,
  funcTable: FuncTable,
  resourceTable: ResourceTable,
  stringTable: UniqueStringArray
  ...
  // let resourceType = null;
  let origin = null;
  const resourceIndex = funcTable._resource[funcIndex];
  if (resourceIndex !== -1) {
    // resourceType = resourceTable.type[resourceIndex];
    const resourceNameIndex = resourceTable.name[resourceIndex];
    origin = stringTable.getString(resourceNameIndex);
...
export function getTreeOrderComparator(
  callNodeTable: CallNodeTable,
  sampleCallNodes: Array<IndexIntoCallNodeTable | null>
): (arg1: IndexIntoSamplesTable, arg2: IndexIntoSamplesTable) => number {
  /**
...
export function filterToDeallocationsMemory(nativeAllocations: BalancedNativeAllocationsTable): NativeAllocationsTable {
  ...
  for (let allocationIndex = 0; allocationIndex < nativeAllocations.length; allocationIndex++) {
    ...
    if (bytes >= 0) {
      ...
      memoryAddressToAllocationSite[memoryAddress] = nativeAllocations.stack[allocationIndex];
      continue;
    }
...
export function extractProfileFilterPageData(
  ...
  if (filteredPages.length > 1) {
    ...
    filteredPages = filteredPages.filter((page) => !page.url.startsWith("about:"));
  }
 ...
 */
export function hasThreadKeys(threadIndexesSet: Set<ThreadIndex>, threadsKey: ThreadsKey): boolean {
...
export function replaceStackReferences(
  thread: Thread,
  mapForSamplingSelfStacks: Map<IndexIntoStackTable, IndexIntoStackTable | null>,
  mapForBacktraceSelfStacks: Map<IndexIntoStackTable, IndexIntoStackTable | null>
): Thread {
  const {
    samples: oldSamples,
    markers: oldMarkers,
  ...
  const samples = {
    ...
    stack: oldSamples.stack.map((oldStackIndex) => {
      ...
      if (newStack === undefined) {
        throw new Error(`Missing mapForSamplingSelfStacks entry for stack ${oldStackIndex}`);
      }
      return newStack;
    }),
  } as const;

  function mapBacktraceSelfStack(oldStackIndex: number | null) {
    if (oldStackIndex === null) {
      return null;
    }
    const newStack = mapForBacktraceSelfStacks.get(oldStackIndex);
    if (newStack === undefined) {
      throw new Error(`Missing mapForBacktraceSelfStacks entry for stack ${oldStackIndex}`);
    }
    return newStack;
  }

  // Markers
  function replaceStackReferenceInMarkerPayload(oldData: MarkerPayload | null): MarkerPayload | null {
 ...
 */
export function findAddressProofForFile(profile: Profile, file: string): AddressProof | null {
  const { libs } = profile;
...
export function getNativeSymbolInfo(
  ...
  const functionSize =
    functionSizeOrNull ??
    calculateFunctionSizeLowerBound(frameTable, nativeSymbols.address[nativeSymbol], nativeSymbol);
  return {
...
export function getBottomBoxInfoForCallNode(
  ...
  const { stackTable, frameTable, funcTable, stringTable, resourceTable, nativeSymbols } = thread;
  ...
  const libIndex =
    resource !== -1 && resourceTable.type[resource] === resourceTypes.library ? resourceTable.lib[resource] : null;
  const nativeSymbolsForCallNode = getNativeSymbolsForCallNode(callNodeIndex, callNodeInfo, stackTable, frameTable);
  const nativeSymbolInfosForCallNode = nativeSymbolsForCallNode.map((nativeSymbolIndex) =>
    getNativeSymbolInfo(nativeSymbolIndex, nativeSymbols, frameTable, stringTable)
  );
...
export function determineTimelineType(profile: Profile): TimelineType {
  ...
  if (!profile.meta.sampleUnits || !profile.threads.some((thread) => thread.samples.threadCPUDelta)) {
