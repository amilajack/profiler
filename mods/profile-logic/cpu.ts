...
import type { Thread, Milliseconds, SampleUnits, ThreadCPUDeltaUnit, Profile } from "~/components/FFP/types";

/**
 * Compute the max CPU delta value per ms for that thread. It computes the
 * max value after the threadCPUDelta processing.
 */
export function computeMaxThreadCPUDeltaPerMs(threads: Thread[], profileInterval: Milliseconds): number {
...
export function computeMaxCPUDeltaPerInterval(profile: Profile): number | null {
  ...
  switch (threadCPUDeltaUnit) {
    case "µs":
    case "ns": {
      const cpuDeltaTimeUnitMultiplier = getCpuDeltaTimeUnitMultiplier(threadCPUDeltaUnit);
      return cpuDeltaTimeUnitMultiplier * interval;
    }
    case "variable CPU cycles": {
      const maxThreadCPUDeltaPerMs = computeMaxThreadCPUDeltaPerMs(profile.threads, interval);
 ...
 */
export function processThreadCPUDelta(thread: Thread, sampleUnits: SampleUnits, profileInterval: Milliseconds): Thread {
  const { samples } = thread;
  const { threadCPUDelta } = samples;

  if (!threadCPUDelta) {
    throw new Error("processThreadCPUDelta should not be called for the profiles that don't include threadCPUDelta.");
  }
  // A helper function to shallow clone the thread with different threadCPUDelta values.
  function _newThreadWithNewThreadCPUDelta(threadCPUDelta?: Array<number | null>): Thread {
    const newSamples = {
      ...samples,
      threadCPUDelta,
    } as const;
  ...
  const cpuDeltaTimeUnitMultiplier = getCpuDeltaTimeUnitMultiplier(sampleUnits.threadCPUDelta);
  ...
  for (let i = 0; i < samples.length; i++) {
    ...
    switch (threadCPUDeltaUnit) {
      ...
      case "variable CPU cycles":
        ...
        break;
      default:
        throw assertExhaustiveCheck(threadCPUDeltaUnit, "Unhandled threadCPUDelta unit in the processing.");
    }
  }
...
function getCpuDeltaTimeUnitMultiplier(unit: ThreadCPUDeltaUnit): number {
  switch (unit) {
    ...
    case "variable CPU cycles":
      ...
      return 1;
    default:
      throw assertExhaustiveCheck(unit, "Unhandled threadCPUDelta unit in the processing.");
  }
}
 ...
 */
function findClosestNonNullValueToIdx(array: Array<number | null>, idx: number, distance: number = 0): number {
