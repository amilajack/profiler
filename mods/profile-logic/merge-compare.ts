...
import { filterRawMarkerTableToRange } from "~/components/FFP/profile-logic/marker-data";
import { adjustMarkerTimestamps, adjustTableTimestamps } from "~/components/FFP/profile-logic/process-profile";
import { filterThreadSamplesToRange, getTimeRangeForThread } from "~/components/FFP/profile-logic/profile-data";
import { ensureExists } from "~/components/FFP/utils/flow";
import { UniqueStringArray } from "~/components/FFP/utils/unique-string-array";
import {
  getEmptyFrameTable,
  getEmptyFuncTable,
 ...
 */
export function mergeProfilesForDiffing(profiles: Profile[]): {
  profile: Profile;
  ...
  const { categories: newCategories, translationMaps: translationMapsForCategories } = mergeCategories(
    profiles.map((profile) => profile.meta.categories)
  );
  resultProfile.meta.categories = newCategories;

  // Then merge libs.
  const { libs: newLibs, translationMaps: translationMapsForLibs } = mergeLibs(profiles.map((profile) => profile.libs));
  ...
  const transformStacks: Record<string, any> = {};
  ...
  for (let i = 0; i < profiles.length; i++) {
    ...
    //   throw new Error("Only one thread selection is currently supported for the comparison view.");
    // }
    ...
    thread.stackTable = {
      ...thread.stackTable,
      category: adjustCategories(thread.stackTable.category, translationMapsForCategories[i]),
    };
    thread.frameTable = {
      ...thread.frameTable,
      category: adjustNullableCategories(thread.frameTable.category, translationMapsForCategories[i]),
    };
    thread.resourceTable = {
      ...thread.resourceTable,
      lib: adjustResourceTableLibs(thread.resourceTable.lib, translationMapsForLibs[i]),
    };
    thread.nativeSymbols = {
      ...thread.nativeSymbols,
      libIndex: adjustNativeSymbolLibs(thread.nativeSymbols.libIndex, translationMapsForLibs[i]),
    };
      ...
      getComparisonThread(translationMapsForCategories, [
        {
          ...
          weightMultiplier: profiles[0].meta.interval / resultProfile.meta.interval,
        },
        {
          thread: resultProfile.threads[1],
          weightMultiplier: profiles[1].meta.interval / resultProfile.meta.interval,
        },
      ])
    );
  }
  ...
  resultProfile.meta.initialVisibleThreads = resultProfile.threads.map((_, i) => i);

  // take minimum sample count from all threads
  resultProfile.meta.sampleCount = Math.min(...resultProfile.threads.map((t) => t.samples.time.length));

  resultProfile.meta.projectId = profiles[0].meta.projectId;
...
function filterThreadToRange(
  ...
  thread.markers = filterRawMarkerTableToRange(thread.markers, derivedMarkerInfo, rangeStart, rangeEnd);
  return thread;
}
...
type TranslationMapForNativeSymbols = Map<IndexIntoNativeSymbolTable, IndexIntoNativeSymbolTable>;
 ...
 */
function mergeCategories(categoriesPerThread: Array<CategoryList | undefined>): {
  categories: CategoryList;
  translationMaps: TranslationMapForCategories[];
} {
  const newCategories: CategoryList = [];
  const translationMaps: TranslationMapForCategories[] = [];
...
function adjustResourceTableLibs(
  // type of ResourceTable.libs
  libs: Array<IndexIntoLibs | null>,
...
function mergeLibs(libsPerProfile: Lib[][]): {
  ...
  const translationMaps: TranslationMapForLibs[] = [];
...
function combineResourceTables(
  ...
  threads.forEach((thread) => {
    ...
    for (let i = 0; i < resourceTable.length; i++) {
      ...
      const newHost = hostIndex !== null ? stringTable.getString(hostIndex) : null;
      ...
      newResourceTable.host.push(newHost === null ? null : newStringTable.indexForString(newHost));
...
function combineNativeSymbolTables(
  ...
  const mapOfInsertedNativeSymbols: Map<string, IndexIntoNativeSymbolTable> = new Map();
  ...
  threads.forEach((thread) => {
    ...
    for (let i = 0; i < nativeSymbols.length; i++) {
      ...
      const insertedNativeSymbolIndex = mapOfInsertedNativeSymbols.get(nativeSymbolKey);
...
function combineFuncTables(
  ...
  funcTable: FuncTable;
  translationMaps: TranslationMapForFuncs[];
} {
  const mapOfInsertedFuncs: Map<string, IndexIntoFuncTable> = new Map();
  const mapOfInsertedHashes: Map<string, IndexIntoFuncTable> = new Map();
  const translationMaps: TranslationMapForFuncs[] = [];
  const newFuncTable = getEmptyFuncTable();

  const parseThread = (thread: Thread, threadIndex: number) => {
    ...
    for (let i = 0; i < funcTable.length; i++) {
      ...
      const newResourceIndex = resourceIndex >= 0 ? resourceTranslationMap.get(resourceIndex) : -1;
      ...
      const _resourceIndex = funcTable._resource[i];
      const _newResourceIndex = _resourceIndex >= 0 ? resourceTranslationMap.get(_resourceIndex) : -1;
      if (_newResourceIndex === undefined) {
        throw new Error(stripIndent`
          We couldn't find the _resource of func ${i} in the translation map.
          This is a programming error.
        `);
      }
      ...
      //    not guaranteed to be unique in a resource. That's why we use the line
      //    number as well.
      // 3. Label frames: they have no resource, only a name. So we can't do
      //    better than this.
      const funcKey = [name, fileName, newResourceIndex, lineNumber, columnNumber].join("#");
      const hash = funcTable?.hash?.[i];
      const insertedFuncIndexFromFuncMap = mapOfInsertedFuncs.get(funcKey);
      const insertedFuncIndexFromHashMap = mapOfInsertedHashes.get(hash ?? "");
      if (hash !== null && insertedFuncIndexFromHashMap !== undefined) {
        translationMap.set(i, insertedFuncIndexFromHashMap);
        continue;
      }
      if (insertedFuncIndexFromFuncMap !== undefined) {
        translationMap.set(i, insertedFuncIndexFromFuncMap);
        continue;
      }
      mapOfInsertedFuncs.set(funcKey, newFuncTable.length);
      if (!!hash) mapOfInsertedHashes.set(hash, newFuncTable.length);
      ...
      newFuncTable._resource.push(_newResourceIndex);
      ...
      newFuncTable.threadOrigin?.push(threadIndex);

      newFuncTable.length++;
    }

    // insert translation map at threadIndex
    translationMaps[threadIndex] = translationMap;
  };

  /**
   * Each func has a unique hash based on name, fileName, resourceIndex, lineNumber and columNumber.
   * When merging threads, a func hash can appear across threads while having different func body.
   * `parseThread` won't merge duplicate func hashes. Therefore, we insert threads in reverse order
   * to make sure funcs from the second thread (the merge thread) are inserted first.
   * This way, if a func hash appears in both threads, threadOrigin will always point to the last thread.
   */
  for (let threadIndex = threads.length - 1; threadIndex >= 0; threadIndex--) {
    parseThread(threads[threadIndex], threadIndex);
  }
...
function combineFrameTables(
  ...
  translationMaps: TranslationMapForFrames[];
} {
  ...
  threads.forEach((thread, threadIndex) => {
    ...
    const getNewCategory = (category: number) => {
    ...
    const nativeSymbolTranslationMap = translationMapsForNativeSymbols[threadIndex];
    ...
    for (let i = 0; i < frameTable.length; i++) {
      ...
      const implementation =
        typeof implementationIndex === "number" ? stringTable.getString(implementationIndex) : null;

      const nativeSymbol = frameTable.nativeSymbol[i];
      const newNativeSymbol = nativeSymbol === null ? null : nativeSymbolTranslationMap.get(nativeSymbol);
...
function combineStackTables(
  ...
  translationMaps: TranslationMapForStacks[];
} {
  ...
  threads.forEach((thread, threadIndex) => {
    ...
    for (let i = 0; i < stackTable.length; i++) {
      ...
      const stackIdIndex = stackTable.stackId[i];
      const stackId = stackIdIndex === -1 ? "null" : stringTable.getString(stackTable.stackId[i]);
      newStackTable.stackId.push(newStringTable.indexForString(stackId));
...
function combineSamplesDiffing(
  ...
  threadsAndWeightMultipliers: [ThreadAndWeightMultiplier, ThreadAndWeightMultiplier]
): {
  samples: SamplesTable;
  translationMaps: TranslationMapForSamples[];
} {
  ...
  const sampleNormalizeFactor = samples2.length / samples1.length;
  ...
  while (i < samples1.length || j < samples2.length) {
    ...
    const nextSampleIsFromThread1 =
      i < samples1.length && (j >= samples2.length || samples1.time[i] < samples2.time[j]);
    ...
    if (nextSampleIsFromThread1) {
      ...
      // Normalize the first profile sample count to match the second
      newWeight.push(-weightMultiplier1 * sampleWeight * sampleNormalizeFactor);
...
function getComparisonThread(
  ...
  const { resourceTable: newResourceTable, translationMaps: translationMapsForResources } = combineResourceTables(
    newStringTable,
    threads
  );
  const { nativeSymbols: newNativeSymbols, translationMaps: translationMapsForNativeSymbols } =
    combineNativeSymbolTables(newStringTable, threads);
  const { funcTable: newFuncTable, translationMaps: translationMapsForFuncs } = combineFuncTables(
    translationMapsForResources,
    newStringTable,
    threads
  );
  const { frameTable: newFrameTable, translationMaps: translationMapsForFrames } = combineFrameTables(
  ...
  const { stackTable: newStackTable, translationMaps: translationMapsForStacks } = combineStackTables(
    translationMapsForCategories,
    translationMapsForFrames,
    newStringTable,
    threads
  );
  const { samples: newSamples } = combineSamplesDiffing(translationMapsForStacks, threadsAndWeightMultipliers);
...
export function mergeThreads(threads: Thread[]): Thread {
  ...
  const newSamples = combineSamplesForMerging(translationMapsForStacks, threads);

  const { markerTable: newMarkers } = mergeMarkers(translationMapsForStacks, newStringTable, threads);
  ...
  for (const thread of threads) {
    processStartupTime = Math.min(thread.processStartupTime, processStartupTime);
    processShutdownTime = Math.max(thread.processShutdownTime || Infinity, processShutdownTime);
    registerTime = Math.min(thread.registerTime, registerTime);
    unregisterTime = Math.max(thread.unregisterTime || Infinity, unregisterTime);
  }

  const mergedThread = {
    processType: "merged",
    processStartupTime,
    processShutdownTime: processShutdownTime === Infinity ? null : processShutdownTime,
    registerTime,
...
function combineSamplesForMerging(
  ...
  while (true) {
    ...
    // the complexity.
    for (let sampleTablesIndex = 0; sampleTablesIndex < sampleTables.length; sampleTablesIndex++) {
      ...
      if (currentSamplesIndex < currentSamplesTable.length && currentSampleTime < time) {
    ...
    const newStackIndex =
      stackIndex === null ? null : translationMapsForStacks[selectedSamplesTableIndex].get(stackIndex);
    if (newStackIndex === undefined) {
      throw new Error(stripIndent`
...
function mergeMarkers(
  ...
  const newThreadId: Tid[] = [];
  const newMarkerTable = { ...getEmptyRawMarkerTable(), threadId: newThreadId } as const;
    ...
    for (let markerIndex = 0; markerIndex < markers.length; markerIndex++) {
      ...
      if (oldData && "cause" in oldData && oldData.cause) {
        ...
        if (newStack === undefined) {
          throw new Error(`Missing old stack entry ${oldStack} in the translation map.`);
        }
        ...
        const urlString = oldData.url === undefined ? undefined : stringTable.getString(oldData.url);

        newMarkerTable.data.push({
          ...oldData,
          url: urlString === undefined ? undefined : newStringTable.indexForString(urlString),
        } as MarkerPayload);
      } else {
        newMarkerTable.data.push(oldData);
      }

      newMarkerTable.name.push(newName === null ? -1 : newStringTable.indexForString(newName));
      ...
      newThreadId.push(markers.threadId ? markers.threadId[markerIndex] : thread.tid);
...
function mergeScreenshotMarkers(
  ...
  const targetMarkerTable = { ...targetThread.markers } as const;
    ...
    if (thread.stringTable.hasString("CompositorScreenshot")) {
      ...
      for (let markerIndex = 0; markerIndex < markers.length; markerIndex++) {
        ...
        if (data !== null && data.type === "CompositorScreenshot") {
          ...
          const urlString = data.url === undefined ? undefined : stringTable.getString(data.url);

          // Move compositor screenshot marker data to the new marker table.
          const compositorScreenshotMarkerData = {
            ...data,
            url: urlString === undefined ? undefined : targetThread.stringTable.indexForString(urlString),
          } as Readonly<MarkerPayload>;

          targetMarkerTable.data.push(compositorScreenshotMarkerData);
          targetMarkerTable.name.push(newName === null ? -1 : targetThread.stringTable.indexForString(newName));
          ...
          if (targetMarkerTable.threadId) {
            targetMarkerTable.threadId.push(markers.threadId ? markers.threadId[markerIndex] : thread.tid);
          }
