...
export function getSchemaFromMarker(
  ...
  return markerSchemaByName[getMarkerSchemaName(markerSchemaByName, markerName, markerData)] || null;
}
...
export function parseLabel(
  ...
  const computeLabelParts: Array<(arg1: Marker) => string> = splits.map((part, i) => {
    if (i % 2 === 0) {
    ...
    if (keys.length === 2) {
      ...
      switch (markerKey) {
        ...
        case "end":
          return (marker) => (marker.end === null ? "unknown" : formatTimestamp(marker.end));
        case "duration":
          return (marker) => (marker.end === null ? "unknown" : formatTimestamp(marker.end - marker.start));
        case "name":
    ...
    if (markerKey === "data") {
      ...
      let format: MarkerFormatType | null = null;
      ...
      return (marker) => {
        ...
        return format ? formatFromMarkerSchema(markerSchema.name, format, value) : value;
      };
    }
...
const fallbacks: Partial<Record<LabelKey, (arg1: Marker) => string>> = {
...
export function getLabelGetter(
  ...
  labelKey: LabelKey
): (arg1: MarkerIndex) => string {
  // Build up a list of label functions, that are tied to the schema name.
  const labelFns: Map<string, (arg1: Marker) => string> = new Map();
  ...
  const getFallbackLabel = ensureExists(fallbacks[labelKey], "Unable to find a fallback label function.");
  ...
  return (markerIndex: MarkerIndex) => {
    ...
    if (label === undefined) {
      ...
      const schemaName = getMarkerSchemaName(markerSchemaByName, marker.name, marker.data);
 ...
 */
export function formatFromMarkerSchema(markerType: string, format: MarkerFormatType, value: any): string {
  if (value === undefined || value === null) {
    console.warn(`Formatting ${value} for ${markerType} with format ${JSON.stringify(format)}`);
    return "(empty)";
  }
  if (typeof format === "object") {
    switch (format.type) {
      case "table": {
        ...
        const rows = hasHeader ? [columns.map((x) => x.label || "(empty)")] : [];
        const cellRows = value.map((row, i) => {
          ...
          if (row.length !== columns.length) {
            throw new Error(`Row ${i} length doesn't match column count (row: ${row.length}, cols: ${columns.length})`);
          }
      ...
      default:
        throw new Error(`Unknown format type ${JSON.stringify(format.type as never)}`);
    }
  }
  switch (format) {
    case "url":
    ...
    case "list":
      ...
      return value.map((v) => formatFromMarkerSchema(markerType, "string", v)).join(", ");
    default:
      console.warn(`A marker schema of type "${markerType}" had an unknown format ${JSON.stringify(format as never)}`);
      return value;
  }
}
              ...
              {value.map((row, i) => {
                ...
                return (
                  <tr key={i}>
                    {row.map((cell, i) => {
                      return (
                        <td key={i}>{formatMarkupFromMarkerSchema(markerType, columns[i].type || "string", cell)}</td>
                      );
                    })}
                  </tr>
                );
    ...
    case "list":
      ...
      return (
        <ul className="marker-list-value">
          {value.map((entry, i) => (
            <li key={i}>{formatFromMarkerSchema(markerType, "string", value[i])}</li>
          ))}
        </ul>
      );
    case "url": {
      ...
      return (
        <a href={value} target="_blank" rel="noreferrer" className="marker-link-value">
 ...
 */
export function markerPayloadMatchesSearch(markerSchema: MarkerSchema, marker: Marker, searchRegExp: RegExp): boolean {
  const { data } = marker;
  if (!data) {
    return false;
  }
