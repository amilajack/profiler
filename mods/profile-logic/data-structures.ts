...
import { GECKO_PROFILE_VERSION, PROCESSED_PROFILE_VERSION } from "~/components/FFP/app-logic/constants";
import type {
  Thread,
  SamplesTable,
  FrameTable,
...
export function shallowCloneFuncTable(funcTable: FuncTable): FuncTable {
  return {
    ...
    _resource: funcTable._resource.slice(),
...
export function shallowCloneNativeSymbolTable(nativeSymbols: NativeSymbolTable): NativeSymbolTable {
  return {
    // Important!
    // If modifying this structure, please update all callers of this function to ensure
    // that they are pushing on correctly to the data structure. These pushes may not
 ...
 */
export const resourceTypes = getResourceTypes() as Readonly<ReturnType<typeof getResourceTypes>>;
