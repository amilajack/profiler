...
import { sortDataTable } from "~/components/FFP/utils/data-table-utils";
import { resourceTypes } from "~/components/FFP/profile-logic/data-structures";
...
import { PROCESSED_PROFILE_VERSION } from "~/components/FFP/app-logic/constants";
import { coerce } from "~/components/FFP/utils/flow";
import type { MixedObject, SerializableProfile } from "~/components/FFP/types";
 ...
 */
export function attemptToUpgradeProcessedProfileThroughMutation(profile: unknown): SerializableProfile | null {
  ...
  if (typeof meta.preprocessedProfileVersion !== "number") {
    ...
    if (!firstThread || typeof firstThread !== "object" || !firstThread.stringArray) {
  ...
  const profileVersion =
    typeof meta.preprocessedProfileVersion === "number" ? meta.preprocessedProfileVersion : UNANNOTATED_VERSION;
...
function _guessMarkerCategories(profile: any) {
  ...
  const keyToCategoryIndex: Map<string, number> = new Map(
    keyToCategoryName.map(([key, categoryName]: [any, any]) => {
...
const _upgraders = {
      ...
      if (extraMarkers.length > 0) {
        ...
        while (nextOldMarkerIndex < markers.length || nextExtraMarkerIndex < extraMarkers.length) {
          ...
          if (nextOldMarkerTime <= nextExtraMarkerTime) {
            ...
            nextExtraMarkerTime =
              nextExtraMarkerIndex < extraMarkers.length ? extraMarkers[nextExtraMarkerIndex].time : Infinity;
          }
        }
  ...
  [12]: (profile) => {
    ...
    const exactMatches = new Map([
      ["-[GeckoNSApplication nextEventMatchingMask:untilDate:inMode:dequeue:]", IDLE],
      ...
      ["mozilla::ThreadEventQueue<mozilla::EventQueue>::GetEvent(bool,mozilla::EventPriority *)", IDLE],
      [
      ...
      ["nsCCUncollectableMarker::Observe(nsISupports*, char const*, char16_t const*)", GCCC],
    ...
    function getCategoryForFuncName(funcName: string): number | undefined {
        ...
        if (categoryBasedOnFuncName !== undefined) {
          ...
          if (oldCategory !== null) {
            if (!funcTable.isJS[funcIndex] && domCallRegex.test(funcName)) {
              ...
              const newCategory =
                oldCategory in oldCategoryToNewCategory ? oldCategoryToNewCategory[oldCategory] : 1; /* Other */
  ...
  [30]: (profile) => {
    ...
    for (const { markers } of profile.threads) {
      ...
      for (let i = 0; i < markers.length; i++) {
        const data: Payload | null | undefined = markers.data[i];
  ...
  [34]: (profile) => {
    ...
    for (const thread of profile.threads) {
      ...
      for (const data of markers.data) {
        if (data && data.type !== "tracing" && data.type !== "Styles" && data.cause && data.cause.time !== undefined) {
  ...
  [35]: (profile) => {
    ...
    if (profile.meta.configuration && profile.meta.configuration.activeBrowsingContextID) {
      profile.meta.configuration.activeTabID = profile.meta.configuration.activeBrowsingContextID;
  ...
  [36]: (profile) => {
    ...
    for (const thread of profile.threads) {
      ...
      frameTable.nativeSymbol = frameTable.func.map((f) => funcToNativeSymbolMap.get(f) ?? null);
  ...
  [38]: (profile) => {
    ...
    for (const thread of profile.threads) {
      ...
      const { samples, markers, jsAllocations, nativeAllocations, stackTable, frameTable } = thread;
      ...
      samples.stack = samples.stack.map((oldStackIndex) =>
        oldStackIndex === null ? null : mapForSamplingSelfStacks.get(oldStackIndex) ?? null
      );
      ...
      if (jsAllocations !== undefined) {
        jsAllocations.stack = jsAllocations.stack.map((oldStackIndex) =>
          oldStackIndex === null ? null : mapForSyncBacktraces.get(oldStackIndex) ?? null
        );
      }
      if (nativeAllocations !== undefined) {
  ...
  [39]: (profile) => {
    for (const thread of profile.threads) {
      if (thread.samples.threadCPUDelta) {
        ...
        const hasCPUDeltaValues = thread.samples.threadCPUDelta.some((val) => val !== null);
  ...
  [41]: (profile) => {
    ...
    for (const thread of profile.threads) {
      const { libs: threadLibs, resourceTable, nativeSymbols, stringTable } = thread;
      ...
      for (let resourceIndex = 0; resourceIndex < resourceTable.length; resourceIndex++) {
        ...
        if (nameStringIndex === -1 || nameStringIndex === undefined || nameStringIndex === null) {
          resourceTable.name[resourceIndex] = stringTable.indexForString("<unnamed resource>");
        }
      ...
      for (let nativeSymbolIndex = 0; nativeSymbolIndex < nativeSymbols.length; nativeSymbolIndex++) {
  ...
  [46]: (profile) => {
    ...
    for (const thread of profile.threads) {
      thread.isMainThread =
        thread.name === "GeckoMain" || typeof thread.pid === "string" || String(thread.pid) === thread.tid;
    }
  },
  [47]: (profile) => {
