...
import type { CallTreeCountsAndSummary } from "~/components/FFP/profile-logic/call-tree";
...
export function getRootsAndChildren(
  ...
  for (; callNodeIndex < callNodeTable.length; callNodeIndex++) {
    ...
    while (i < end) {
      if (
        array[i + 1] !== 0 &&
        funcName > thread.stringTable.getString(thread.funcTable.name[callNodeTable.func[array[i + 1]]])
      ) {
  ...
  roots.sort((rootA, rootB) => {
    const [nameA, nameB] = [rootA, rootB].map((callNodeIndex) =>
      thread.stringTable.getString(thread.funcTable.name[callNodeTable.func[callNodeIndex]])
    );
    if (nameA < nameB) {
      return 1;
    }
...
export function getFlameGraphTiming(
  ...
  const { callNodeChildCount, callNodeSummary, rootTotalSummary } = callTreeCountsAndSummary;
  ...
  while (stack.length) {
    ...
    const totalRelative = Math.abs(callNodeSummary.total[nodeIndex] / rootTotalSummary);
    const selfRelative = Math.abs(callNodeSummary.self[nodeIndex] / rootTotalSummary);
